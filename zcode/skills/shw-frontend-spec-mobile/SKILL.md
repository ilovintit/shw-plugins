...
遵守 `shw-frontend-spec-common` 的图标铁律：禁止 Emoji，只允许 font-icon。统一 `<ShwIcon :name="<语义名>" />`（classPrefix 默认 `shwkj`）；名称来自项目映射与公司批准的 Lucide 快照，不能直接依赖上游文件名。图标字体、`@font-face`、语义名称映射和组件入口由 `@shwkj/taro-ui` 与 `shw-ci-icons` 集中维护；业务页面不得散落 codepoint、字体地址或另建 `@font-face`。

微信原生 `tabBar.iconPath` / `selectedIconPath` 必须使用 `shw-ci-icons` 从**同一语义映射和同一 Lucide SVG**生成的 PNG：普通态与选中态只改变配置色，不得手绘、下载或选用不同图形。页面 font-icon 与 TabBar PNG 是同一图标库的两种产物；TabBar 图片路径只写进 `app.config.ts`，不在页面引用。
...
- 提交按钮无防双击
- 使用 Emoji、字符、图片或 SVG 充当图标，或绕过 `ShwIcon` 在业务页面自建字体图标
