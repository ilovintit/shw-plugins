---
name: shw-rbac
description: 管理后台 / 内部系统 RBAC 权限系统的设计规范。在设计或实现后台权限系统、角色 / 权限点 / 数据范围、组织架构树、菜单权限时触发。覆盖 8 张表、权限点代码化（非 DB 表）、PermissionRegistry 权限树、5 档数据范围、多角色合并算法、权限检查三层分层、幽灵码自愈、超管短路、两级缓存。用户提到 RBAC、权限系统、角色权限、数据范围、行级权限时自动触发。
---
...
`PermissionRegistry` 提供两种视图：**按分组组织的权限树**（供前端权限分配面板渲染）与**扁平合法码列表**（供后端校验与幽灵码自愈）。
...
读取管理员权限码时，**过滤掉不在 Registry 中的非法码**，并**自动删除 DB 残留**。这保证"删功能"后，DB 里的残留权限码不会误授权，且自我修复无需人工干预。
...
超管判定只读会话里的 `role` 字段，**不依赖任何 RBAC 表**：
...
| 普通管理员 | 所有角色权限码合并去重后的列表（经幽灵码自愈清洗） |
...
由专门的 `CodeRule` 声明（前缀、长度、字符类型、填充），项目里所有业务编码统一走这套规则。
...
| 幽灵码自愈：过滤 + 自动删 DB 残留 | 任由 DB 残留已删功能的权限码 |
