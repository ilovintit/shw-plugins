...
- 读每个摘要
- 验证实现互不冲突
- 跑快速层验证（单元测试/lint/typecheck/build）——不跑 E2E/VRT 重型层，由夜间 shw-e2e-vrt-audit 巡检承接
- 集成所有改动
...
```
子智能体 1 → 实现 Domain 层
子智能体 2 → 实现 Application 层
子智能体 3 → 实现 Interface 层
子智能体 4 → 实现 Infrastructure 层
[等待全部完成]
主智能体 → 注册路由（依赖前 4 个的类名）
```
...
3. **跑快速层验证** - 验证所有实现协同工作（不跑 E2E/VRT 重型层）
