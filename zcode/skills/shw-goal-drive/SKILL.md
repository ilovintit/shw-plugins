...
**完成判定（单判据）**：全部 Issue closed + 各 PR CI 全绿 + checklist 全勾，且每项经主 agent 亲自核对（Issue 状态、CI run 链接、checklist 勾选）。不以"轮次做够了"或"评论够多了"为判据。goal 完成、目检项（若有）汇总移交用户后，提示用户运行 `/shw-review`（双模型对抗终审，范围带上本 Milestone）——终审 🔴 清零出放行结论后，走 `/shw-release` 放行（建 dev→main 放行 PR 描述列 Milestone 内 Issue 清单，交用户确认合并），main 合并后打 tag 发布——**线上验证通过后才关 Milestone**（Sprint Review 核对时确认，口径见 shw-gitea-flow §5）；skill 不自动终审、不合 main，发布决策是用户确认域。
