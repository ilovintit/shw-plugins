...
description: 公司设计规范（DDD 四层架构见 shw-ddd；错误处理体系、RBAC 权限、后台任务、会话管理等组件规范见公司 lib 仓库文档，shw-lib-docs 指路）在 Go/GoFrame（GoFrame v2）项目里的落地实现指南。把架构/流程规范翻译成具体 Go/GoFrame 代码时触发：项目分层落地、手工依赖注入、gerror+gcode 错误处理落地、Controller 极薄实现、DAO/Entity 用法、decimal string 金额精度、Go 命名约定。最近 go.mod 含 github.com/gogf/gf/v2 时适用。
---
...
## 3. Controller 落地：极薄 + 规范路由（架构规范：shw-ddd；权限规范：公司 lib 仓库文档）

Controller 的职责边界、参数校验分层、数据范围归属等**架构规范**定义在 **shw-ddd**；权限检查的三层分层、数据范围档位、权限点定义等**组件规范**定义在公司 lib 仓库文档（**shw-lib-docs** 指路）。本节写这些规范在 Go/GoFrame Controller 层的**落地写法**。
...
| 权限判断（Authorization） | 中间件 | 认证 + 权限点检查，Controller 不做（三层分层见 lib 仓库 rbac 文档） |
| 数据范围（Data Scope） | UseCase | 每个 UseCase 方法按入口定义自己的数据范围（档位定义见 lib 仓库 rbac 文档） |
...
rbac 数据范围规范（lib 仓库文档）的落地方式：按调用入口定义不同 UseCase 方法，每个方法内部校验数据范围。Controller 保持极薄，数据范围规则集中在 UseCase 易测试、易复用。
...
权限检查三层分层由 rbac 规范定义（lib 仓库文档）。Go/GoFrame 落地：路由级权限点由中间件统一检查；个别需要行级 / 字段级细控的，在 Controller 或 UseCase 里显式调 `rbac.Check(ctx, perm)`。**不要**在 Controller 里手写角色判断。
...
## 4. 错误处理落地：gerror + gcode detail 反推（设计范式：公司 lib 仓库文档）

统一响应四字段（traceId / code / message / data）、错误三分类（Domain / Application / Infrastructure → HTTP 422 / 200 / 500）、错误码编号规则、日志分级等**设计范式**定义在公司 lib 仓库文档（**shw-lib-docs** 指路）。本节聚焦这些范式在 Go/GoFrame 项目里的**落地写法与代码组织**。
...
```go
// internal/shared/errtype/errtype.go —— ErrorType（对应错误三分类，见 lib 仓库错误处理文档）
...
```go
// 统一响应中间件 —— 落地统一响应四字段规范（lib 仓库错误处理文档）
...
错误码集中在一个文件，用 Go 的 `var()` 块统一定义（Go 的"枚举类"），放 `internal/shared/code/codes.go`。编号规则（层前缀 + 模块前缀 + 序号）的设计参见公司 lib 仓库错误处理文档。
...
登录态、token 认证、操作者身份存取的**设计范式**（Redis Hash + Set 双结构、UUID token、滑动过期、多角色实例化、Bearer 中间件注入 context）定义在公司 lib 仓库 session 文档（**shw-lib-docs** 指路）。Go/GoFrame 落地要点：

**构造函数注入（多角色参数化）**：Go 用构造函数注入 Role/TTL/IDField 三参数，与 PHP 的模板方法模式等价（lib 仓库 session 文档「参数注入两种落地形态」），在 `module.go` 手工装配：
