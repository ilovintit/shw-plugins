...
│   ├── assets/                   # 端内图片与font-icon字体；依赖许可和版本可追溯
...
- `<entry>`沿用项目按业务角色×实际端确定的稳定名称，如 admin-pc、user-h5、user-wechat-mini；不包含Issue号、worktree名、日期或发布版本。H5、每类小程序和 iOS/Android/鸿蒙的已确认入口分别标注交互与验收状态；同一产品流程可链接，但不默认共用一套实现或交互。旧 Taro 项目按现有入口保留并逐步迁移。
...
PC原型按PC规范表达查询区、操作区、表格、分页和弹窗；移动 H5/小程序按该端已发布 shw-ui 能力及真实平台规范表达，原生 App 三端分别按其系统交互表达。图标使用已核对的端内资源与语义映射，不跨端复制字体或微信 TabBar 图形。优先用可运行的真实组件资产；HTML模拟必须记录对应组件与已核对文档，不能改变组件契约或替代生产实现。
