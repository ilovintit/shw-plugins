...
发布模板遵循 `shw-gitea-ci/references/project-template.md`：`dev` 始终供本地开发者工具调试；无 test 环境时 dev 同时发布体验版，有 test 环境时由 test 发布体验版，main/tag 发布正式版。dev→main 的全量检查是正式测试位置；main 打 tag 不重复测试，完成来源、版本和制品校验后直接发布。全量失败或未完成时，只有用户明确授权才可带例外继续，并保留真实证据。
