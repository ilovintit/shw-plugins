...
后端动作：
  0. 白名单校验：directory 必须命中服务端预置的归档目录白名单，未登记的一律拒绝
...
**directory 必须过服务端白名单**：归档目录是服务端预置的封闭集合（各业务的归档目录在服务端登记，如 `avatars` / `order-attachments` / `documents`），前端传来的 directory 只是"从中选择"，不是"任意指定"。签发票据前必须校验 directory 命中白名单，未登记值直接拒绝——防路径穿越（`../../` 类输入拼进对象存储 key 逃逸到任意路径）、防公私目录混淆（把私有业务文件传进公开 CDN 目录，绕过预签名直接裸奔可读）。绝不允许前端任意指定未登记目录。
...
interface StorageInterface:
    // 签发上传（directory 必须已通过服务端归档目录白名单校验，见阶段 1；拒绝任意路径）
