...
**两项均无默认值，都需要填写**。面板值经清单插值注入 env，清单条目拉起 shim 网关（后端 pin 版本不在清单条目里，随插件构建配置固定；文中出现的 `vX.Y.Z` 均为占位示例，**实际固定版本见 src/plugin/mcp-shim/backend-version.json**——升级 = 修改 backend-version.json + 重跑 capture + 发版，shim 启动时自动对齐新版本后端）：
