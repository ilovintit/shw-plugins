...
3. **基线维护**：`baseline-maintenance.yml` 只接受 `/shw-baseline` 产生的独立维护 Issue 上下文、不可变 accepted source、最终人工验收引用与 create/sync 范围。普通 push/PR、回归失败或 release workflow 均无写权限。生成候选、人工范围审查、同目标复跑后才提交基线 PR。
