...
插件的 `gitea` MCP server 拉起的是官方 gitea-mcp（Go 实现，`go run` 固定版本拉起——`vX.Y.Z` 占位示例，实际固定版本见插件构建配置（config.md 有说明），版本随插件发版升级，插件不打包二进制），通过 `GITEA_HOST` / `GITEA_ACCESS_TOKEN` 两个环境变量寻址与认证目标 Gitea——这是各 agent 工具共同的底层机制，配置入口因工具而异（见「前置条件」与同目录 config.md 第 1 节）。gitea-mcp 二进制本身没有任何内置默认值：这两个环境变量未配置时认证必然失败（`token is required` 等错误），不会"默认连上某个 Gitea"。

**核心原则**：

1. 声称"代码正常"之前，先确认 CI 通过。本地测试通过 ≠ CI 通过。
2. 🛑 **合并绝对禁令**：合并决策权归用户，agent 永远不执行合并——`pull_request_write` 的 `merge` 与 `merge_when_checks_succeed` 两个 method 一律不调（后者是 auto-merge 变体，同属合并）。用户在对话中明确要求合并时也不执行，只给 PR 链接引导用户到 Gitea UI 手动操作，并说明这是纪律设计而非工具能力缺陷。
...
2. gitea MCP 的 GITEA_HOST / GITEA_ACCESS_TOKEN 已配置（入口按所用工具，细节见同目录 config.md 第 1 节对应小节）
   - ZCode：插件配置面板填 gitea_host + gitea_token 两项（Settings → Plugin Management → shw-plugins）
   - Kimi Code：用户级 ~/.kimi-code/mcp.json 写同名 gitea 条目整体覆盖插件声明
   - OpenCode：清单以 {env:VAR} 占位符承接 shell 环境变量，启动前 export
   - 未配置时 gitea MCP 认证必失败，配置探测见下方「首次使用探测」
...
- **返回认证错误**（如 `token is required`）→ 判定为未配置，**立即停止后续 gitea 调用**（不要换别的 gitea 工具继续试，不要撞墙后才排障），按当前工具给出最短配置路径：
  - ZCode → 插件配置面板补填 gitea_host + gitea_token 两项（config.md 第 1 节「ZCode：插件配置面板」）
  - Kimi Code → 在 ~/.kimi-code/mcp.json 写同名 gitea 条目，host + token 均填（可粘贴模板见 config.md 第 1 节「Kimi Code：用户级 mcp.json 同名条目覆盖」）
  - OpenCode → 在启动 shell 里导出两个环境变量（config.md 第 1 节「OpenCode：shell 环境变量承接」）
...
- PR 创建后**由用户在 Gitea UI 审核并手动合并**——合并执行是核心原则第 2 条的**绝对禁令**，任何情况下（含用户在对话中明确要求时）agent 都不执行合并，只给 PR 链接
...
- 🛑 **合并类 method 一律不调**——`merge` / `merge_when_checks_succeed` 均属合并操作（见核心原则合并绝对禁令），合并决策归用户，等用户在 Gitea UI 手动合并
...
2. 目标是 test / main 等受保护分支 → 移交本 skill 场景 5 开 PR（受保护分支，等待用户手动合并；绝不 `git push` 直推）
