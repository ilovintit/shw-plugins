...
`shw-icons check --config deploy/icons/config.json` 校验映射、codepoint 和配置中列出的 TabBar PNG 文件存在性。页面图标名称、字体/CSS/组件产物和 `app.config.ts` 的业务引用须由项目测试/人工验收覆盖；在工具实现相应语义解析前不得宣称由本命令验证。生成或检查失败不得回退 Emoji、字符、手工 PNG 或 SVG 组件。
