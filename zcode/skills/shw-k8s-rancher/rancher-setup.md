...
### 第 3 步：.gitignore 检查（防 token 入库，必执行）

```bash
# 1. 确认项目 .gitignore 覆盖 .rancher/
grep -n "^\.rancher/" .gitignore

# 2. 没有则追加（.gitignore 文件不存在时先创建）
echo ".rancher/" >> .gitignore

# 3. 验证：kubeconfig 不得出现在未跟踪列表
git status --short
```

第 3 步验证若 `.rancher/kubeconfig` 出现在 `??` 未跟踪条目里：说明 ignore 未生效（检查写入行拼写、检查全局 / 上级目录 gitignore），修好再继续。**kubeconfig 入库 = token 泄漏，本步不可跳过。**
