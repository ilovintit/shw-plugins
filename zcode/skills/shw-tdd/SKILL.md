...
确认：
- 测试通过
- 其他测试仍通过（回归范围 = 快速层：单元测试/lint/typecheck；重型层回归如 E2E 端到端/VRT 视觉回归不在此循环内，由夜间巡检 shw-e2e-vrt-audit 承接）
- 输出干净（无错误、警告）

**测试失败？** 改代码，不改测试。

**其他测试失败（快速层：单元/lint/typecheck）？** 立即修。重型层回归（E2E 端到端/VRT 视觉回归）不在此循环内，由夜间巡检承接。
