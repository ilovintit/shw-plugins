...
更新还必须识别项目是否存在真实 test 环境，并把小程序上传器、配置与 workflow 一起迁移为唯一匹配的二分模式：无 test 时 `channels`、push branches 和 job if 全部是 dev/main，dev 同时承担本地开发者工具和体验版来源；有 test 时三处全部是 test/main，dev 仅供本地开发者工具、test 提供体验版。不得保留 dev/test/main 混合 channels 或 workflow 分支集；模式不匹配时按 uploader 的受控迁移提示修复。dev 手动全量测试入口只能在用户明确授权后执行；dev→main 承担正式全量测试，main/tag 不重复测试而直接发布。
