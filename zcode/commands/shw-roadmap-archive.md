...
6. **Git commit（自动提交，不 push）**
   ...
   2. 是 git 仓库且有变更则提交归档产生的变更（`.roadmaps/` 下的 rename 与轨迹内容变更，含 spec 补同步产生的 specs/ 变更）：
      ```bash
      git add .roadmaps/
      git add specs/   # 若 spec 有补同步变更
      git commit -m "归档 roadmap: <name>（完成）"
      ```
      放弃归档的 message 为 `归档 roadmap: <name>（放弃）`。
   3. **不执行 `git push`**

   **异常处理：**
   - 非 git 仓库 → 跳过提交，在摘要中注明 "非 git 仓库，跳过提交"
   - 没有变更需要提交 → 跳过提交，在摘要中注明
