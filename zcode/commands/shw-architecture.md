...
1. 关联产品定义 Issue，读取 PRD、所有已确认的公司规范驱动的高保真HTML原型及其可选实现映射；有界面但视觉、状态或交互未完整覆盖时先回 `/shw-prototype`。
...
Kubernetes 应用同时加载 shw-release-flow/references/environments.md 与 shw-test-spec/references/ci-config.md；deployment.md 必须给出分支触发、受影响服务映射、镜像到 Fleet 更新闭环、固定开发集群/域名、环境中间件职责、HTTP Ingress 与外部前置。

开发部署架构须核对 shw-backend-stack/references/middleware-images.json 的 verified 引用、平台和凭据前置；所需中间件先缓存再部署。缓存就绪不代表PVC、应用连接或集群部署已经验收。

微信小程序发布架构必须加载 shw-release-flow/references/miniprogram/README.md，映射真实AppID/入口/编译目录/API环境/robot/私钥责任、插件镜像digest及上传证据。SDK上传成功与微信审核/正式发布、页面和真机验收分开；不把独立发布工具当业务Dockerfile基础镜像。
