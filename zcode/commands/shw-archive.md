...
4. **评估 delta spec 同步状态**
   ...
   用 status JSON 的 `artifactPaths.specs.existingOutputPaths` 检查 delta spec。若不存在，不带同步提示继续。

   **若有 delta spec：**
   - 提示用户是否要先运行 `/shw-sync` 同步 delta spec 到主 spec
   - 提供选项：
     - "立即同步（推荐）" → 先运行 `/shw-sync <name>`，完成后再继续归档
     - "跳过同步直接归档"

   不自行做 delta→主 spec 的映射或合并预览，统一交给 `/shw-sync` 处理。
