...
把空目录或尚未形成实质产品内容的新项目初始化为当前生命周期工作流。执行前加载 `shw-gitea-flow`、`shw-gitea-repo`、`shw-product-docs`。
...
加载 `shw-gitea-ci/references/project-template.md`。空项目初始化时必须让用户选择 `issue-main`、`issue-dev` 或 `managed`，并生成 `.shw/project.yaml`；不检测、复用或迁移现有模式。公共 shw-ui/Go 库只在对应端已有正式包时通过私有包源按版本引用，不创建公共组件 submodule；未发布时记录缺口，不编造包名。若目录已有实质代码、文档、Git 流程或模式标识，停止并提示使用 `shw-update`。
...
5. 仅 `managed` 建立标准 Gitea labels、分支保护、工程 PR Gate、dev 非阻断回归入口、最终验收后的显式基线维护入口、CI 镜像和部署契约；轻量模式只记录项目自身 CI 与 Issue/PR 证据。任何模式都不得把 API/E2E/VRT 设为开发或发布前置。
...
H5 与每种小程序按真实平台分别建立原生工程和构建输出，不为新项目生成 Taro 双端默认配置。微信项目同步开发者工具 miniprogramRoot、自动化与上传路径；H5 独立维护部署/归档路径与清理范围。将“项目正常调试时改代码保持打开、复用自动化连接和 watch，不反复关闭重开”写入微信项目 Agent 规范。iOS、Android、鸿蒙仅在产品确有该端时建立各自原生入口；不得假定 shw-ui 提供其组件或统一交互。

### 统一图标初始化（#195）

含 Web/小程序前端入口时加载 `shw-frontend-spec-common/references/icon-management.md`，按实际支持的端建立 `deploy/icons/config.json`；每个接入字体链的端声明业务语义名→已批准的 Lucide 名称、font family/prefix 与产物位置，微信原生 TabBar 再声明普通/选中色和同源 PNG。项目不自行安装字体生成器；CI 使用已验证的 `shw-ci-icons` digest。原生 iOS/Android/鸿蒙不强制采用此字体/微信 TabBar 机制，按各系统原生资产规范单独设计和验证。
