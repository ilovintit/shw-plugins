---
description: 初始化空项目/新项目为 Issue 驱动工作流——询问 → 目录骨架 → AGENTS.md → Gitea 流程设施。已有存量项目请走 /shw-import
...
- **流程**：需求入口 `/shw-issue` 建 Issue（backlog label），agent 侧由 `shw-issue-gate` skill 兜底（无单不开工），单入口三分支——
  ...
  - 三层数点相同：PR（`Closes #N`，目标 dev）→ CI 门禁 → **用户人工验收 → 归档环节合并**（交互模式；roadmap/goal 编排模式 CI 全绿自动合并）；发布前 `/shw-review` 双模型对抗终审 → `/shw-release` 放行（dev→main PR，用户确认合并，main 仅用户）
