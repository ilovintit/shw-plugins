...
# Fixed events only; no paths, env values or arguments. stderr remains host-owned.
launch_log() {
  printf '[gitea-mcp-launch] time=%s pid=%s event=%s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$$" "$1" >&2
}
launch_log launcher_start
...
launch_log launcher_user_env_loaded
...
launch_log launcher_workspace_env_loaded
...
if [ ! -x "$bin" ]; then
  bin=$(sh "$dir/../runtime/download-binary.sh" "gitea-mcp-shim-$os-$arch") || exit $?
fi

# 发行物可能缺执行位：补一次（容忍失败，如只读安装场景）；失败则由 exec 自然报错。
chmod +x "$bin" 2>/dev/null || true

launch_log launcher_exec
exec "$bin" "$@"
