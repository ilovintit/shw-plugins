...
async function waitForEvent<T>(
  emitter: { on: (event: string, cb: (payload: T) => void) => void; off: (event: string, cb: (payload: T) => void) => void },
  eventType: string,
  timeoutMs = 5000
): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const onEvent = (payload: T) => { clearTimeout(timer); emitter.off(eventType, onEvent); resolve(payload); };
    const timer = setTimeout(() => { emitter.off(eventType, onEvent); reject(new Error(`等待事件 ${eventType} 超时（${timeoutMs}ms 后）`)); }, timeoutMs);
    emitter.on(eventType, onEvent);
  });
}
