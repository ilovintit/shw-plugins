...
description: Version/Milestone 规划公共机制。按用户价值拆分 Issue，建立依赖顺序与人工验收计划；回归基线在最终验收后独立维护。
---
...
- Issue 必须包含范围、非目标、依赖与人工可判定的验收标准；适用工程检查单列。
- 依赖构成有向无环图；roadmap 按拓扑顺序严格串行。
- 用户决定范围、优先级和拆分边界；Agent 提供风险与建议。

## 验收映射

每条 AC 对应人工步骤、预期结果、角色、数据和环境，并反向指向 PRD/原型。可以记录未来回归保护候选，但不得在开发前生成 API/E2E/VRT 用例或把自动化完备性设为 ready 条件。最终人工验收后，`/shw-baseline` 以独立 chore Issue/PR 固化明确确认的范围。

## 状态

```text
planning → ready → developing → dev-tested
→ release-pending → released → final-accepted → baseline-pending|baseline-recorded → completed
```
