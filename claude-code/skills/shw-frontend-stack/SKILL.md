...
- 所有端遵守 `shw-frontend-spec-common` 的图标铁律：禁止 Emoji，只允许 font-icon，并通过端基座的统一组件按语义名称使用。
- 移动端使用 `shw-frontend-spec-mobile` 规定的 `ShwIcon` 与包内字体。
- PC/Nuxt 沿用各端基座的统一 font-icon 组件；参考 `xlzb-project`、`ccl-new-project` 的 `IconFont` 方式，集中维护字体资源、`@font-face`、语义名称映射和组件入口，业务页面不得直接管理字体细节。
