...
1. 从真实微信小程序原生工程入口盘点每个AppID、编译目录、API环境、robot、上传私钥归属；AppID/路径/settings写项目代码，不做统一数量的全局CI变量。上传器不依赖 Taro，但只支持微信标准小程序，不自动覆盖其他小程序平台。
...
配置中的 `channels` 是项目真实 test 环境的唯一上传模式声明，且只能二选一：没有 test 环境时严格配置 `dev/main`，dev 上传体验版；有 test 环境时严格配置 `test/main`，test 上传体验版，dev 只供本地开发者工具调试。不得同时写 dev/test/main，也不得只配置一个分支。workflow 的 push branches 与 job if 必须使用同一对分支；示例默认无 test 的 `dev/main`，接入有 test 的项目时两处一起替换为 `test/main`。预检遇到模式不匹配会返回受控的环境、允许分支及迁移提示：新建 test 环境后迁移为 `test/main`；撤销 test 环境后迁移为 `dev/main`。main 上传代码不等于微信审核或正式发布，正式发布仍须用户在微信后台完成。robot范围1–30，示例dev=3/test=2/main=1只是通道模式，按项目事实配置；robot不是环境或审核权限，API地址由编译配置决定。多个workflow共享AppID时使用同一并发组，避免同时上传；不要仅靠目录path过滤漏掉共享组件/lockfile/编译配置变化。模板默认不做路径过滤，项目可补经过验证的影响映射。
...
程序拒绝PR、未配置的分支及 fleet 分支上传、混合或不完整的环境模式、缺失或修改的build记录、工作区逃逸/符号链接、bundle内私钥/认证文件、错误robot及私钥格式/权限。分支与环境模式不匹配时，受控错误会指明 without-test/with-test、允许分支和对应迁移方向；不输出配置全文、私钥、SDK原始错误或原始响应。上传在隔离子进程中运行，SDK原始stdout/stderr不转发；失败 JSON 仅 allowlist `code`、外层 `sdkCode`、微信内层 `wechatErrCode`、清洗后的 `errMsg` 和可判断的 `stage`（`ticket` / `upload` / `source-validation`）。`errMsg` 会移除控制字符、截断到 240 字符，并脱敏 private key、ticket、token、签名、Authorization 与 Cookie 等疑似凭证。上传最多15分钟，失败/中断非零退出，外层不自动重试。

排查时先看 `stage` 确定 ticket、上传或源码校验阶段，再用 `wechatErrCode` 与清洗后的 `errMsg` 判断微信拒绝原因；`sdkCode` 只表示 SDK 外层错误，不能单独当作微信响应码。`-10008 / invalid ip` 检查 runner 出口 IP 与微信后台白名单，`-80011 / get new ticket fail` 检查 ticket 获取链路、账号状态和微信服务；`source-validation` 检查编译产物、源码与包体限制。结构化诊断增强不表示上传已经恢复，消费项目仍须单独升级 [image.json](image.json) 的 digest 并真实复跑，且不得在日志中补开 vendor debug 或输出 Secret。
