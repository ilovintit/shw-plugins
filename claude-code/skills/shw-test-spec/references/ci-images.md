...
1. **workflow 执行环境**：插件维护 reg.shw.top/library/shw-ci-node、shw-ci-go、shw-ci-playwright、shw-ci-rust，供 Gitea job/container 或 CI 测试服务容器中的工具执行。包含检查、编译、测试工具；仅用于 CI。
...
| shw-ci-rust | Rust 1.85.1、wasm32-wasip1、Git/curl、musl-dev；固定引用 `reg.shw.top/library/shw-ci-rust@sha256:0a0e5d7c68ecea45b0ca4f74992441cca2722ba1184f7eb1168ff571f686b2b9`，禁止在 job 内 rustup 安装工具链或 target |
