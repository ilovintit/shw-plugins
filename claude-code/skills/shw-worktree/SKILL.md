...
2. `inspect` 核对文件和分支；调用 `remove(id, claim_id)`。MCP 会检查路径归属、Git 登记、脏文件并 fetch 来源分支证明 HEAD 已合入；只有本地来源分支可安全快进时，才移除工作区和本地 Issue 分支，随后把本地 `dev`（Hotfix 为 `main`）快进到对应 `origin/*`。本地来源分支脏、分叉或无法安全更新时保留 Issue 工作区与分支，不执行 reset/force。
...
5. 远端 Issue 分支和 Issue 状态仍通过原交付流程处理；MCP 不替代 Gitea。remove 成功后复查远端 Issue 分支、本地 Issue 分支、工作区，以及本地来源分支与 `origin/*` 是否一致。
