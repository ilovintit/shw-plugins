...
```json
{"root":"/absolute/path/to/worktrees", "auto_prune": true}
```

`auto_prune` 省略时为 true；设为 false（或环境变量 `SHW_WORKTREE_AUTO_PRUNE=0`）关闭启动后的自动清理，手动 prune 不受影响。
...
├── state.lock                 # 状态文件跨进程锁；不要手动删除
├── locks/<id>.lock            # 单个工作区的跨进程锁；不要手动删除
├── prune.lock / prune.stamp   # 自动清理互斥与 24 小时节流；不要手动修改
...
- **Kimi Code**：清单不携带 `env`，没有插件设置项，也没有 `${VAR}` 插值机制；`SHW_WORKTREE_ROOT` 只能从父进程环境继承。推荐直接用上述用户级 `worktree.json`，不必每个项目各配一次。
