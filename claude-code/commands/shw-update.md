...
检测：项目直接依赖 `lucide-static`，复制 `tools/font-icons/`、`svgtofont` 或其他字体生成器；页面散落 `@font-face`、codepoint、Emoji、字符或 SVG 图标；移动端 TabBar PNG 手工维护、普通态/选中态图形不同，或没有与页面图标共享语义映射。

修复：加载 `shw-frontend-spec-common/references/icon-management.md`。盘点每个真实端的现有图标名称、字体资源、页面引用和 TabBar 配置，再建立 `deploy/icons/config.json`：业务语义名映射到公司批准的 Lucide 名称，明确端内字体/组件输出以及 TabBar 的输出、颜色和图标清单。删除重复工具前先由 `shw-ci-icons` 在该项目 CI 中生成并核验 WOFF2、CSS、组件与 PNG；页面改为统一 `IconFont`/`ShwIcon`。不直接升级图标源，项目不安装 `lucide-static`；公共镜像版本和内部 Lucide tag 只能由插件维护者升级。静态资源发布凭据保留 Gitea Secret，禁止写入图标配置或产物。
