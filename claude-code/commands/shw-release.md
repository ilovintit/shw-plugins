...
4. 创建唯一 dev→main 放行 PR，正文列出版本范围、Issue、镜像/迁移、风险、回滚和人工测试证据；确认当前候选已触发适用工程、来源与发布检查。API/E2E/VRT 不进入放行聚合。
5. 停止并明确提示用户在 Gitea Web UI 审查和手动合并，同时报告工程/发布检查真实状态。已有基线回归可作为参考，但差异、未运行或缺基线不是例外合并债务。Agent 不调用 main 合并 API，也不跳过、取消或伪造检查。

### 阶段 B：合并后发布

用户合并后再次运行同一命令：

1. 验证 PR 确已合并，main HEAD 与批准的候选 commit 对齐；不采信口头状态。读取该候选适用工程、来源与发布检查的真实结果，不把 API/E2E/VRT 报告当放行状态。
2. 在 main 合并提交创建并推送版本 tag；若 token 无 tag 权限，输出精确人工操作而不伪造完成。
3. 跟踪发布 CI，分别核验源 tag、CI job、Harbor/发布制品等证据。
4. 输出发布事实并指向 `/shw-deploy`；不把 tag、镜像或部署健康等同于最终人工验收。用户最终确认后才可运行 `/shw-baseline`。
