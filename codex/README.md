...
- **普通 Issue**：新建 Codex chat 时选择 **Worktree**，起点选 `dev`；开工先创建/使用 `feat/<N>-<slug>`，保持 Issue:分支:Worktree 1:1。
- **Roadmap / Milestone**：使用 Codex **permanent worktree** 承载整个里程碑，逐 Issue 换分支；不要用一次性 managed worktree 承载长循环。
