...
- 测试策略统一由shw-test-spec定义：Issue→dev只跑改动相关模块/文件测试，禁止全测和未知映射时全量兜底；同仓库dev→main必须实际启动全量检查。只有同一发布PR的内容未变 main 回灌，可按同步证据规则复用该 PR 已成功的全量结果；不得复用定向、其他 PR 或任意同 tree 结果。
- main 分支保护必须保留“仅用户 Web UI 合并”，但不得把全量自动化配置成用户无法覆盖的绝对 required-check 阻断。检查照常运行并保留失败状态；用户可基于紧急性或人工验证结果例外合并。Agent 没有该覆盖权，且必须记录例外证据与测试债务。
...
- Workflow 按 `shw-test-spec` 显式选择 `ci-fast` 或 `ci-heavy` runner：快速定向门禁、发布构建和镜像构建使用 `ci-fast`，重型/全量测试使用 `ci-heavy`，均不能依赖随机落池的兼容标签。
...
确认 labels、dev/main、分支保护、合并后删分支、Actions 和敏感文件忽略规则；main 保护同时核对“仅用户 Web UI 合并”和“用户可覆盖全量检查结果”，不得授予 Agent/API 合并权。按产品形态检查镜像/deploy/Fleet 或包分发/安装升级。无管理权限时输出需用户完成的精确设置和验证方法。
