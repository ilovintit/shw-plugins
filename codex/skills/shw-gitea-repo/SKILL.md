...
- 工程检查策略由项目真实栈定义：Issue→dev和dev→main运行构建、lint/typecheck、适用单元/协议/安全检查；未知映射失败并补齐，不全仓兜底。API/E2E/VRT 按 shw-test-spec 走 dev 独立非阻断回归与最终验收后的基线维护，不进入 required contexts。
- main 分支保护保持“仅用户 Web UI 合并”及适用工程/来源检查。不得残留已移除的三类测试 context 造成永久 pending，也不得授予 Agent/API main 合并权。
...
确认 labels、dev/main、分支保护、合并后删分支、Actions 和敏感文件忽略规则；main 保护核对“仅用户 Web UI 合并”、工程 required contexts 及三类测试旧 context 已清除。按产品形态检查镜像/deploy/Fleet 或包分发/安装升级。无管理权限时输出需用户完成的精确设置和验证方法。
