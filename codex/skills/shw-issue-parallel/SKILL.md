...
description: 由 shw-plugins 插件的 apply 主 Agent 在面临 2+ 独立任务时调用，决定并行委派策略。判断哪些任务（验收 checklist 条目/AC）可并行、哪些必须串行。含本地静态确认/CI 测试分层纪律（agent 与子智能体不跑任何测试）。
---
...
- **前端思考 / 实现类**（涉及 .vue / .tsx / Taro / Element Plus / Nuxt 等）→ 子智能体 `model` 优先设为 `kimi-k3`；不可用时保持默认模型（静默降级，不阻断）。口径见 `shw-frontend-stack` §模型路由偏好。
- **其余任务**（后端 / 基础设施 / 文档）→ 默认模型（不指定 model 覆盖）。
