---
name: shw-archive
description: "Codex prompt workflow for shw-archive、/shw-archive. 归档已完成的 change——合并 PR（核对 CI 全绿、用户确认）→ 执行轨迹回写关联 Issue → 本地 .changes/ 草稿清理 Use only when the user explicitly asks for this named workflow."
---

# shw-archive

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-archive`、`/shw-archive` 或要求执行该工作流时按下方原文执行。

**参数**：[change名]

归档已完成的 change：核对交付状态 → 合并 PR（用户确认下核对 CI 全绿并合并）→ 执行轨迹回写关联 Issue（经 gitea-mcp 评论）→ 清理本地 `.changes/` 草稿。本命令是 change 族交付的最后一环：合并就发生在归档环节内——apply 之后仍可继续修改，直到归档时才把 PR 落主干。归档**不产任何 specs 产物、不做 spec 合并**——多真相源问题不存在；留痕只有两处：Issue 评论（执行轨迹）与 git 主干（PR/merge commit）。本命令与 /shw-apply 同属**执行段**——只依赖三件套固化产物，可委派给无探索上下文的其他 agent/会话。
...
- **合并必须用户确认**——agent 不静默合并任何 PR；本命令管 change 族（有三件套）的合并归档，微活链的合并收尾归 /shw-wrap
