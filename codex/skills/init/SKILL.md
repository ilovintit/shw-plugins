...
加载 `shw-gitea-ci/references/project-template.md`。空项目初始化时必须让用户选择 `issue-main`、`issue-dev` 或 `managed`，并生成 `.shw/project.yaml`；不检测、复用或迁移现有模式。公共 shw-ui/Go 库只在对应端已有正式包时通过私有包源按版本引用，不创建公共组件 submodule；未发布时记录缺口，不编造包名。若目录已有实质代码、文档、Git 流程或模式标识，停止并提示使用 `update`。
