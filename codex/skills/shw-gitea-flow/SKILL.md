...
先按 `shw-issue-gate/project-mode.md` 读取项目模式。以下生命周期表只适用于 `managed`；`issue-main` 使用 Issue 分支→main，`issue-dev` 使用 Issue 分支→dev→main，均不自动套用业务项目目录、全量测试、Fleet 或发布规范；无标识项目不主动进入本 Skill。
...
| 交付 | 一个功能 Issue + 一个分支 + 一个交付 PR | 普通项经工程检查进入 dev；Hotfix 按下表完成 main 与 dev 同步 |
| 生产 | tag + 制品 + 适用部署/分发 + 最终人工验收 | 用户确认后关闭 Milestone |
| 回归基线 | 独立 chore Issue + dev PR + manifest | 固化已最终确认范围，不改变历史发布 |
...
- 交付 Issue：由 `/split` 从 Version 拆出，可独立验收。
- Bug/Hotfix：由 `/bug` 从 dev、生产或使用反馈产生。
- 基础设施/维护：chore，不得混入无关产品 Issue。
- 基线维护：最终人工验收后的 chore，引用 Version/功能 Issue/验收证据与不可变 source；只含测试、fixture、expected 和元数据。
...
- 产品文档、适用工程测试、实现和必要部署声明进入同一个功能 PR。API/E2E/VRT 基线不得提前强绑功能 PR；最终验收后使用独立基线维护 Issue/PR。
...
- 基线 PR 目标 dev，进入后随未来正常发布传播到 main；不得为追齐基线改写旧 tag 或自动创建发布。

## 命令边界

- `/version` 创建范围；`/split` 创建多个 Issue；`/test-plan` 固定验收映射。
- `/work` 交付一个 Issue；`/roadmap` 串行调度多个 Issue。
- `/release` 创建放行 PR并在人工合并后打 tag；`/deploy` 只读观测；`/version-close` 经用户确认关闭 Milestone。
