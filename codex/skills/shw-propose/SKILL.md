---
name: shw-propose
description: "Codex prompt workflow for shw-propose、/shw-propose. 固化 explore 的探索结论为 change 三件套（proposal/design/tasks）——.changes/ 本地草稿，gitignore。前置：同会话已有 explore 差距调查结论（无则先 /shw-explore，本命令不现想方案） Use only when the user explicitly asks for this named workflow."
---

# shw-propose

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-propose`、`/shw-propose` 或要求执行该工作流时按下方原文执行。
