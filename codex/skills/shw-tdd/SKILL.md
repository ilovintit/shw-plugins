---
name: shw-tdd
description: 在写任何实现代码前调用。测试先行（RED-GREEN-REFACTOR），红绿由 PR CI 判定——agent 本地不跑任何测试，只做编译/类型检查。
---
...
先写测试。写最小代码让它通过。**红绿由 PR CI 判定**——agent 本地不跑任何测试（本地只做编译/类型检查），push 后 CI 是唯一裁判。

**核心原则**：如果测试从未在 CI 红过，你不知道它是否在测对的东西——CI 的红历史就是证明。

**本地/CI 边界（无例外）**：

- **本地只做编译级确认**：build / typecheck，秒级、无环境依赖——确认没有语法/类型错误就推送
- **lint 与一切测试（单元/集成/API/E2E/VRT）一律在 PR CI 执行**——agent 不在本地跑任何测试，不因跑测试卡住开发流程
- **交互模式**：提 PR 后即收尾，不等 CI；测试失败由用户发起排查（读 CI 日志修复再推）
- **roadmap/goal 编排模式**：由编排层等 CI 结果红绿循环（见 shw-goal-drive）
- **工作流内分工**：走 Issue 驱动工作流时，RED（写测试）由 /shw-test-draft 前置起草、/shw-test-review 对齐审查，apply 负责 GREEN（实现让用例转绿）；本 skill 的完整循环适用于工作流外独立写码与流程外小修
...
digraph tdd_cycle {
    rankdir=LR;
    red [label="RED\n写失败测试\n+断言自审", shape=box, style=filled, fillcolor="#ffcccc"];
    green [label="GREEN\n最小代码\n+编译确认", shape=box, style=filled, fillcolor="#ccffcc"];
    ci [label="CI 判定\npush 后跑红绿", shape=diamond];
    refactor [label="REFACTOR\n绿后清理", shape=box, style=filled, fillcolor="#ccccff"];
    next [label="下一个", shape=ellipse];

    red -> green;
    green -> ci;
    ci -> refactor [label="绿"];
    ci -> green [label="红：读 CI 日志修"];
    refactor -> ci [label="再 push\n保持绿"];
    refactor -> next;
    next -> red;
}
```
...
**强制推理步骤，替代本地跑测试。** 写完测试不运行——逐断言自问：

- 功能缺失时，这个断言会失败吗？（失败，不是编译错/typo）
- 失败原因正是要测的行为缺失吗？
- 断言测的是真实行为，不是 mock 存在？

**断言在功能缺失时不会失败的**——你在测已有行为或测了个空。改测试。
...
写完做**编译/类型检查**（本地唯一允许的验证）：语法/类型错误当场修，确认干净即提交推送——测试红绿交给 CI。

### CI 判定（GREEN 确认）

**提交推送后由 PR CI 裁决**——agent 本地不跑测试：

- **CI 绿** → 该测试真实有效，进 REFACTOR
- **CI 红** → 读 CI 日志（shw-gitea-ci）：测试失败 = 改代码不改测试（最小实现没到位）；lint 失败 = 按 lint 修。修复后再推，循环到绿

红绿循环的等待方按模式分：交互模式由用户看结果发起排查；roadmap/goal 编排模式由编排层等 CI 自动循环。
...
- [ ] 每个新函数/方法都有测试
- [ ] 每个测试做过断言自审（功能缺失时会失败，失败原因正确）
- [ ] 写了让每个测试通过的最小代码
- [ ] 编译/类型检查干净（本地唯一验证）
- [ ] 已提交推送，CI run 已触发（交互模式不等结果，红绿由 CI 判定）
...
发现 bug？写复现它的失败测试。走 TDD 循环（红绿由 CI 判定）。测试证明修复并防止回归。
