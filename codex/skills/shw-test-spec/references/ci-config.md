...
| 任务 | runner | 判定依据 |
| --- | --- | --- |
| Issue→dev、dev→main 构建、lint、适用单元/协议/安全检查 | `ci-fast` | 工程门禁优先快速反馈，不含 API/E2E/VRT |
| dev 已有基线完整回归报告 | `ci-heavy` | 独立非阻断工作流，默认比较全部适用基线 |
| 最终验收后基线生成与复跑 | `ci-heavy` | 显式维护入口，绑定 accepted source 与验收证据 |
| 发布构建、Docker/Buildx 镜像构建 | `ci-fast` | 制品发布链要求及时调度，不进入重型测试队列 |
| 浏览器回归及其他重型诊断 | `ci-heavy` | 与工程/发布门禁分离，partial 必须显式标识 |
...
## Actions artifact 兼容镜像

Gitea Actions 的 artifact 服务不是 GitHub Actions 的 v3 uploader 的兼容实现。业务 workflow **不得**使用已退役的 `actions/upload-artifact` 或 `actions/download-artifact` 内部镜像；它们可以在 Web UI 显示文件，却可能不会生成可由 Gitea REST API/MCP 枚举和下载的 artifact 记录。

统一使用以下已核验的 Gitea 兼容内部镜像，且必须保留完整不可变 SHA：

| 用途 | `uses` 引用 |
| --- | --- |
| 上传 artifact | `https://git.shw.top/actions/gitea-upload-artifact@ffc5e248cc483620d776d7f8f2e18b8c2ccdd1e5` |
| 下载 artifact | `https://git.shw.top/actions/gitea-download-artifact@d551ff504fe0064fc99d534e832f8372dd61dd42` |

这两个镜像分别同步自 `ChristopherHX/gitea-upload-artifact` 与 `ChristopherHX/gitea-download-artifact`；它们是对 `actions/upload-artifact`、`actions/download-artifact` 的 Gitea 兼容 fork。镜像仅保留 action 运行所需源码、`dist/`、许可证和说明，已移除 GitHub 专用自动化。升级必须由新的维护 Issue 同步来源、清理上游自动化、核对默认分支/固定 SHA，并在真实消费方 CI 中验证上传后能经 Gitea API/MCP 列出和下载。切换仅保证未来上传；旧通用 uploader 产生而未登记到 REST API 的 artifact 不会被追溯修复。

## 项目代码负责的配置

项目前缀、Namespace、各 API/Web/H5 域名、业务入口数量和各环境 ConfigMap 名称写入项目部署声明；不统一成 PROJECT_SLUG、DEV_API_DOMAIN 等 CI 后台变量。Fleet 生成资源不得包含 Kubernetes Secret 或应用运行配置的 Secret 引用，运行配置统一来自 ConfigMap；唯一例外是 `imagePullSecrets` 对基础设施预置拉取凭据名称的引用，项目不得创建该 Secret 或写入其内容。这不改变 Actions/依赖代理等 CI 凭据继续由 Gitea Secret 注入的规则。
