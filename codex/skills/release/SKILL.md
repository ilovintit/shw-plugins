---
name: release
description: "Codex prompt workflow for release、/release. 发布放行链固化——收集版本/Milestone 放行范围与 CI 证据，核对双模型终审，自动创建 dev→main 放行 PR；用户确认合并后，经确认在 main 合并 commit 打 tag 并跟踪 release workflow 与发布仓库结果 Use only when the user explicitly asks for this named workflow."
---
...
**参数**：[版本号或Milestone（可选，缺省取当前Milestone；也可指定版本区间）]
...
2. **核对对抗终审**
   ...
   - 在当前会话、关联 Issue 评论或放行记录中查找 `/review` 的「对抗终审放行」结论，且范围必须覆盖本次 release 基线
   - 未找到可核对的终审结论 → 停止并提示先运行 `/review <同一放行范围>`
...
- **只做放行链**：不替代 `/review`，不修复终审发现，不执行线上验证，不提前关闭 Milestone
