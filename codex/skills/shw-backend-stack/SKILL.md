...
- **开发环境**：固定开发 K8S 内由项目通过 Fleet 自管全部中间件、初始化与持久化；不依赖 person/rdev 或本地共享实例。小程序本地编译连接开发 API。
- **test/生产**：连接运维 Agent 维护的外部中间件；环境声明隔离，不能继承 dev 中间件资源。详见 [环境契约](../shw-release-flow/references/environments.md)。
...
- 缓存中的队列/会话数据按项目和环境隔离；Issue PR 不向共享 dev 发布，CI 使用独立临时实例，不能污染开发数据。
...
workflow 的 `services:` 块按下表起依赖，健康检查就绪前测试步骤不开跑；实际引用使用已验证的内部版本或 `@sha256:<digest>`：

| 中间件 | 镜像 | 健康检查 | 空实例就绪耗时 |
|--------|------|---------|---------------|
| PostgreSQL 18 | `<Harbor>/ci-cache/postgres:<固定18.x版本>` | `pg_isready` | 3~8 秒（含 initdb） |
| Valkey | `<Harbor>/ci-cache/valkey:<固定版本>` | `valkey-cli ping` | 1~2 秒 |
...
- E2E / VRT 需要对象存储时使用内部镜像起独立 silo 测试实例；纯业务用例能否 mock 由 test-plan 明确，不复用 dev/test/生产的业务对象存储。
...
开发环境使用 silo、PostgreSQL 18、RabbitMQ、Valkey 前，必须加载 [缓存契约](references/middleware-cache.md) 和 [固定镜像清单](references/middleware-images.json)，先由专用维护 CI 缓存到 reg.shw.top/ci-cache 并验证，再由业务 Agent 将已验证的内部 digest 写入 dev Fleet 声明。缓存不自动引入组件或升级实例；RabbitMQ 按项目已确认需求使用。平台、认证、额外初始化镜像和未完成项必须明确，不能现场拉公网或把模板存在当作缓存已就绪。
