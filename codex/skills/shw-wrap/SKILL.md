---
name: shw-wrap
description: "Codex prompt workflow for shw-wrap、/shw-wrap. 微活收尾一条龙——回写 Issue + 提 PR + 盯 CI + 全绿自动合并；用户发 wrap 即验收授权。触发词：收尾、回写、提 PR、合并 Use only when the user explicitly asks for this named workflow."
---

# shw-wrap

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-wrap`、`/shw-wrap` 或要求执行该工作流时按下方原文执行。
