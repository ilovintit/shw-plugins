---
name: shw-milestone
description: "Codex prompt workflow for shw-milestone、/shw-milestone. Sprint 计划——开 Milestone，从 status/待开发 label 的 Issue 里挑，大活拆分裁决，为每个 Issue 生成验收 checklist 写回 Issue 描述。 Use only when the user explicitly asks for this named workflow."
---

# shw-milestone

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-milestone`、`/shw-milestone` 或要求执行该工作流时按下方原文执行。
...
下一步：测试用例起稿 `/shw-test-draft`（本 Milestone 全部 Issue）→ 对齐审查 `/shw-test-review`（红黄清零放行）→ `/shw-roadmap` 执行驱动（逐 Issue explore → propose → apply → archive，合并进 dev）→ `/shw-review` 终审 → dev→main 放行 PR（用户确认合并）→ tag 发布。
```
...
- **本命令不搬代码、不开分支**——那是 /shw-apply 的事；Issue 挂 Milestone 发生在本命令挑选确认时，挪"开发中"发生在 apply 领单时
