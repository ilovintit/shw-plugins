...
更新还必须识别项目是否存在真实 test 环境：无 test 时 dev 同时承担本地开发者工具和体验版来源；有 test 时 dev 仅供本地开发者工具、test 提供体验版。dev 手动全量测试入口只能在用户明确授权后执行；dev→main 承担正式全量测试，main/tag 不重复测试而直接发布。
