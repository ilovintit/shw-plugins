...
修复：先保留全部现有测试、expected 与历史记录，解除开发/发布前置，再按真实目的拆成工程检查、dev 对已有基线的独立非阻断比较、最终人工验收后的显式 `/baseline` 维护三条路径。工程 required checks 保留构建、lint/typecheck、适用单元/协议/安全检查；回归报告真实区分差异、无基线、partial、执行错误和目标漂移，但不汇入 required/release gate。基线写入必须绑定不可变 acceptedSourceSha、制品/部署 revision、最终验收引用与 accepted scope，生成候选经审查和同目标复跑后走独立 chore Issue/PR。
