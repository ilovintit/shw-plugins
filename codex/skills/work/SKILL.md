---
name: work
description: "Codex prompt workflow for work、/work. 完成一个且仅一个 Issue——调查、计划、适用工程验证、实现、自审、PR、CI、合入 dev 与轨迹回写全链路。 Use only when the user explicitly asks for this named workflow."
---
...
单 Issue 交付入口。执行前加载 `shw-delivery`、`shw-gitea-flow`、`shw-worktree`、`shw-pr-review`、`shw-gitea-ci` 和 `shw-verify`；仅在存在适合测试先行的工程行为时加载 `shw-tdd`。
...
3. 判断工程 TDD 是否适用：适用时先提交最小断言到同一草稿 PR并确认目标行为 RED；不适用时记录原因与人工 AC。API/E2E/VRT 不作为功能开发前置或 RED 证据。
