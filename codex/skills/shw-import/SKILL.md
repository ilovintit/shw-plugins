---
name: shw-import
description: "Codex prompt workflow for shw-import、/shw-import. 存量项目导入 Issue 驱动工作流——工作流设施对齐 + specs 整理写入 Issue + 历史 Milestone 驱动真相源回补（PRD/HTML 高保真原型/技术文档）。空项目请走 /shw-init Use only when the user explicitly asks for this named workflow."
---

# shw-import

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-import`、`/shw-import` 或要求执行该工作流时按下方原文执行。
