---
name: shw-proto-draft
description: "Codex prompt workflow for shw-proto-draft、/shw-proto-draft. 起草/增改功能 HTML 原型——读 Issue 与 PRD 反推界面结构→高保真交互原型（结构/字段/四态枚举齐全）→用户验收交互逻辑→落盘 docs/design/<功能>.html→提 docs PR（Refs #N） Use only when the user explicitly asks for this named workflow."
---

# shw-proto-draft

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-proto-draft`、`/shw-proto-draft` 或要求执行该工作流时按下方原文执行。
...
原型是**界面真相源**：表结构与 API 契约从界面反推（`/shw-arch-draft` 依赖本产物），VRT 基线以此为锚。纯后端活（无界面）不需要本命令——直接 `/shw-arch-draft`。

---

**输入**：`/shw-proto-draft` 后的参数是 Issue 编号（优先——可反查 PRD 关联与时间线裁决）或功能名（对应 `docs/design/<功能名>.html`）。
...
1. **前置校验与领单**
   ...
   - 定位 PRD 对应章节（`docs/prd/<模块>.md`）：不存在 → 提示先 `/shw-prd-draft`（原型从 PRD 反推，没有需求就没有界面），停止
   - 关联 Issue 仍带 `backlog` label → 打 `status/方案中` 摘 `backlog`（文档三件套领单）；已在 `status/方案中` 则不动
   - 已有原型 → **追加模式**：先通读现有 HTML，只增改涉及界面，不重排未受影响部分；识别出需求本身要改时提示走 `/shw-prd-draft`（增改需求），不在原型里私改需求
...
4. **提 docs PR**
   ...
   - 汇报：原型路径、PR 链接；提醒下一步 `/shw-proto-review`（建议）或直接 `/shw-arch-draft`（必做）
