...
| removing | 同 claim 重试 remove 完成剩余清理；中断可能已先解除 Git 工作区登记而目录仍残留，该中间态同样续走删目录/删分支/同步来源分支，inspect 的 UNREGISTERED_WORKTREE 提示不代表需要人工介入（#312）；reconcile 只报告缺失/剩余分支，不代替检查后删除 |
