...
| UI 库 | `@shwkj/taro-ui` | 组件与视觉约束以 shw-frontend-spec-mobile 为唯一入口 |
...
- **共享库职责分明**：`taro-ui`（mobile 组件库）、`shared-utils`（工具函数）。公司已发布库通过私有包源按固定版本接入，锁文件和 CI 认证随项目维护；只有项目自有、尚未发布且必须共同开发的源码包才可采用明确登记的 workspace/submodule，不能把公司库复制或本地替换进业务仓库。
...
- 所有端遵守 `shw-frontend-spec-common` 的图标铁律：禁止 Emoji，只允许 font-icon，并通过端基座的统一组件按项目语义名称使用。图标源固定为 `shw-forks/lucide` 的公司批准 tag，字体/TabBar 图片统一由 `shw-ci-icons` 构建；项目不安装或自行升级 `lucide-static`。
- 移动端使用 `shw-frontend-spec-mobile` 规定的 `ShwIcon` 与包内字体。
- PC/Nuxt 沿用各端基座的统一 font-icon 组件；公共工具集中维护字体资源、`@font-face`、语义名称映射和组件入口，业务页面不得直接管理字体细节。
- 各端可采用不同的字体包、资源托管和组件实现，但不能改用 Emoji、字符、图片或 SVG 图标体系，也不把某端的字体地址和 codepoint 复制到另一端。
...
上述固定技术栈用于生产工程。验收用HTML原型按shw-design-spec直接在docs/design维护，不因此新建另一套生产应用；组件外观和交互仍服从公司规范，真实框架/组件/接口验收不能由HTML模拟替代。prototype由当前宿主Agent直接执行，下述子智能体偏好不要求为原型另派一个任务。
...
- ❌ **跨端复制同一字体包、地址或 codepoint** → 各端可独立集中维护 font-icon 实现，但都必须遵守通用图标铁律。
