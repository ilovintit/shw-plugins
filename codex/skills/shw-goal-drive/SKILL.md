...
本 skill 是"编排打法手册"，组合两样东西：**Milestone**（`/shw-milestone` 产出的计划：装着已排期 Issue，各 Issue 有验收 checklist）与**原生 goal**（所用 agent 工具的 goal/任务循环能力）。与 `/shw-roadmap` 的关系：roadmap 是交互模式的手动执行驱动，本 skill 是 goal 模式的同构打法——驱动从手动变自动，纪律完全一致。

**加载本 skill 后第一件事就是确认就绪**——不许跳过、不许凭记忆假设：

| 检查项 | 不满足时的动作 |
|---|---|
| Milestone 存在且有 open Issue（经 gitea-mcp 读） | 先走 `/shw-milestone` 排期 |
| 测试用例已起草并过 `/shw-test-review` 放行 | 先走 test 族（/shw-test-draft → /shw-test-review） |
...
```
以 Gitea Milestone <名> 的完成为判据（全部 Issue 关闭 + 各 PR CI 全绿 + 验收 checklist 全勾）：逐 Issue 走 change 族（/shw-explore 差距调查 → /shw-propose 同会话固化 → /shw-apply 实现提 PR → 编排层等 CI 红绿循环到绿后合并进 dev → /shw-archive 回写），每个 Issue 合并后亲自核对状态与 CI 再进下一个
```
...
2. **explore → propose（同会话）**：`/shw-explore <N>` 差距调查（Issue × docs 方案 × 代码现状）→ 紧跟 `/shw-propose` 固化三件套——探索结论不落盘，两步必须同会话衔接
3. **apply → 等 CI → 合并**：`/shw-apply` 实现提 PR（测试用例已由 test 族起稿并审过，apply 做转绿）；**编排层盯 CI run**——绿则合并**进 dev**（编排模式 agent 可自动合 dev，无人值守循环的前提；agent 永不合 main），红则读日志（shw-gitea-ci）修复再推，循环到绿；合并后 `/shw-archive` 回写轨迹
...
**完成判定（单判据）**：全部 Issue closed + 各 PR CI 全绿 + checklist 全勾，且每项经主 agent 亲自核对（Issue 状态、CI run 链接、checklist 勾选）。不以"轮次做够了"或"评论够多了"为判据。goal 完成、目检项（若有）汇总移交用户后，提示用户运行 `/shw-review`（双模型对抗终审，范围带上本 Milestone）——终审 🔴 清零出放行结论后，建 **dev→main 放行 PR**（描述列 Milestone 内 Issue 清单）交用户确认合并，main 合并后打 tag 发布——skill 不自动终审、不合 main，发布决策是用户确认域。
...
**无原生 goal 循环时的退化**：退化为**手动多轮对话**——用户每轮说"继续"，agent 每轮按四步纪律走一个 Issue 并汇报状态与证据（等价于手动跑 /shw-roadmap）；objective 模板作为整个会话的任务书贴在对话里即可。
