---
name: issue
description: "Codex prompt workflow for issue、/issue. 需求入口处理——接需求建 Issue，澄清对话后把背景/目标/约束/初步范围整理进 Issue，判定微活/小活/大活并指路后续命令链。 Use only when the user explicitly asks for this named workflow."
---

# issue

这是共享源码中 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `issue`、`/issue` 或要求执行该工作流时按下方原文执行。
...
**输入**：`/issue <需求描述 | Issue 编号>`。执行前先加载 `shw-gitea-flow` skill。
...
- 项目没初始化过工作流（无 `<!-- shw-workflow:v6 -->` 标记）：检测目标目录是否有实质内容——空项目/新项目提示跑 `/init`，已有代码的存量项目提示跑 `/import`
...
- **微活**提示微活链（`/do <N>` → `/wrap <N>`，wrap 含回写、提 PR、盯 CI、全绿自动合并）：
...
```
/prd-draft → /proto-draft（有界面时）→ /arch-draft
```

**顺序不可反**：表结构/API 契约从界面反推，技术方案永远是三件套最后一个。每块各有 review 命令（`/prd-review`、`/proto-review`、`/arch-review`）——审出问题经你裁决直接修正，主动增改走各 draft 的追加模式；三件套齐后再 `/milestone` 排期 → test 族起稿审查 → `/roadmap` 执行。过程中被否的方案与踩坑按 `shw-work-journal` 记入 `docs/journal/issue-N.md`。
