...
需要保留测试报告、VRT baseline、trace、视频或其他 CI artifact 时，workflow 使用 [Gitea artifact 兼容镜像](../../shw-test-spec/references/ci-config.md#actions-artifact-兼容镜像) 的固定 SHA。上传成功后，artifact 必须可由 Gitea REST API 及 Gitea MCP 枚举、按 ID 下载；只在 Web UI 可见不构成可自动取证的 artifact 交付。不得回退到通用 `actions/upload-artifact` / `actions/download-artifact` 镜像。

## 小程序环境映射

`dev` 始终用于本地开发者工具调试。

- 没有 test 环境：`dev` 同时对应体验版；上传器 `channels`、workflow push branches 和 job if 必须同为 `dev/main`。
- 有 test 环境：`test` 对应体验版，`dev` 不自动上传体验版；上述三处必须同为 `test/main`。
- `main/tag` 对应正式版。

小程序构建和上传绑定准确 Source SHA，私钥只通过 Gitea Secret 临时注入；失败或结果未知不自动重试。`update` 必须先识别真实 test 环境，不能为了套模板强行创建 test。
