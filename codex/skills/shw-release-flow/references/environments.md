...
所有环境的 Fleet 部署统一只使用 ConfigMap 承载应用配置和中间件连接参数，禁止生成或引用 Kubernetes Secret/SecretList，包括 `secretKeyRef`、`secretRef`、Secret volume、`secretName` 与 `imagePullSecrets`。工作负载使用 `configMapKeyRef` 或 `configMapRef` 读取。dev 的 ConfigMap 连同自建中间件账号和密码由项目仓库/Fleet 全量维护；这些中间件仅在开发集群项目范围内使用、不向集群外开放，并允许随环境清空重建。test/生产也遵循 ConfigMap 载体约定，但实际值仍按环境隔离、限制仓库和日志可见范围，不因 ConfigMap 非加密而公开传播。CI、源码仓库或镜像仓库认证继续使用 Gitea Actions Secret 等流水线机制，它们不属于 Fleet 生成的 Kubernetes 资源。
