...
生产从用户人工合并 main 后的稳定 vX.Y.Z tag 发布；程序验证 tag 与 S 相等、S 属于 main。dev→main 运行适用工程、来源与发布检查；API/E2E/VRT 不进入 GitOps 放行。程序不把“存在 tag”、部署健康或回归报告当成最终人工验收。
...
- 输出验证 YAML、资源身份、重复资源、镜像 digest；拒绝 Secret/SecretList 资源及承载运行配置的 Secret 引用（`secretKeyRef`、`secretRef`、Secret volume、`secretName`）、明显私钥和遗留镜像占位符。`imagePullSecrets` 不在拒绝范围内：它只引用基础设施预置的拉取凭据名称，不承载运行配置。所有环境的运行配置统一由 ConfigMap 提供，工作负载使用 `configMapKeyRef`/`configMapRef`；dev ConfigMap 由项目仓库/Fleet 全量维护。该验证不是完整 Kubernetes API schema/业务兼容性检查，也不能证明任意 ConfigMap 文本无敏感信息；test/生产仍需限制配置值的仓库与日志可见范围。
