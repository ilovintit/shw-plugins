---
name: wrap
description: "Codex prompt workflow for wrap、/wrap. 微活收尾一条龙——回写 Issue + 提 PR + 盯 CI + 全绿自动合并；用户发 wrap 即验收授权。触发词：收尾、回写、提 PR、合并 Use only when the user explicitly asks for this named workflow."
---

# wrap

这是共享源码中 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `wrap`、`/wrap` 或要求执行该工作流时按下方原文执行。

**参数**：<Issue 编号>

微活收尾一条龙：读 Issue → 回写验收 checklist 与实施纪要 → 提 PR → 盯 CI → **全绿自动合并** → 删分支。适用于无三件套的微活链（一行样式、小修、流程外小改动）——一个命令跑完 change 族"提 PR + 合并 + 归档"的全部收尾动作。

**核心语义（用户裁决）**：**用户发 `/wrap` 指令本身 = 验收授权。** wrap 提 PR 后**不等人工确认**——盯到 CI 全绿即自动合并（agent 可合 dev，永不合 main）。一行样式没有人工测试量，发 wrap 就是用户拍板。
...
- 核对当前分支 = `feat/N-slug`（bug 用 `bug/N-slug`；hotfix 用 `hotfix/N-slug`——从 main 切，其余从 dev 切）；不在对应分支 → 停下向用户确认（`/do` 在 worktree 干的活，先进该 worktree 目录 `../<repo>-issue-N` 再核对）
...
- **清理 issue worktree**：本次活干在 `../<repo>-issue-N`（/do 强制形态）→ 合并删分支后从主仓库路径清理：`git -C <主目录> worktree remove ../<repo>-issue-N && git -C <主目录> branch -d feat/N-slug`（`git -C` 使清理不依赖会话当前目录——会话仍挂在 worktree 内时同样成立，清理后本会话工作目录即失效，属预期收尾）；remove 不可行（有未迁移文件等）→ 收尾报告列出待清理路径与命令，交用户处置
...
动手前先查 `.changes/` 下是否有本 Issue 关联的三件套草稿（proposal.md / design.md / tasks.md）——**有 → 说明走错了链**：该活是 change 族大活，微活不该有三件套。停下向用户说明，提示改走 `/archive`，**不静默归档、不继续 wrap**。
