---
name: import
description: "Codex prompt workflow for import、/import. 存量项目导入 Issue 驱动工作流——工作流设施对齐 + specs 整理写入 Issue + 历史 Milestone 驱动真相源回补（PRD/HTML 高保真原型/技术文档）。空项目请走 /init Use only when the user explicitly asks for this named workflow."
---

# import

这是共享源码中 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `import`、`/import` 或要求执行该工作流时按下方原文执行。

**参数**：[项目路径，默认当前目录]

把**已有代码的存量项目**接入 Issue 驱动工作流：先对齐工作流设施，再把存量 specs（或从代码结构推导的模块清单）整理写入 Issue，建一个**历史 Milestone** 作为回补容器——逐 Issue 回补真相源（PRD、HTML 高保真 UI+UX 原型、技术文档），回补完成后旧 specs 退役。空项目初始化走 `/init`；已接入项目的插件升级迁移走 `/update`。
...
- AGENTS.md 追加 v6 工作流区段（`<!-- shw-workflow:v6 -->` 标记，模板见 /init 步骤 4；已有旧 change 管理章节则整段替换）
...
- 上述 Issue 全部挂上，每个 Issue 描述末尾追加**回补清单**：
  ...
  ```markdown
  ## 真相源回补清单
  - [ ] PRD：docs/prd/<模块>.md（走 /prd-draft 追加模式）
  - [ ] HTML 原型：docs/design/<模块>.html（有界面的模块；走 /proto-draft）
  - [ ] 技术文档：docs/architecture/<模块>.md（走 /arch-draft）
  ```
