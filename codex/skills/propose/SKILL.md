---
name: propose
description: "Codex prompt workflow for propose、/propose. 固化 explore 的探索结论为 change 三件套（proposal/design/tasks）——.changes/ 本地草稿，gitignore。前置：同会话已有 explore 差距调查结论（无则先 /explore，本命令不现想方案） Use only when the user explicitly asks for this named workflow."
---

# propose

这是共享源码中 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `propose`、`/propose` 或要求执行该工作流时按下方原文执行。

**参数**：[change名 | Issue 编号]

把 explore 的探索结论**固化**为 change 三件套。change 是**执行层的本地草稿**（`.changes/` 目录，gitignore 不入库）——计划在这里落成文，交付经 PR 进主干，归档时轨迹回写 Issue。

**固化是本命令的唯一职责**：方案的产地是 /explore（差距调查），本命令不探索、不现想——没有探索结论就先回去探索。
...
准备好实现时，运行 `/apply`；交付完成后 `/archive` 归档回写。

---

**输入**：`/propose` 后的参数是 change 名（kebab-case）或 Issue 编号。

**步骤**

1. **前置检查：explore 结论在吗**

   - 同会话刚跑过 /explore、差距清单与执行路径在上下文中 → 继续
   - **没有探索结论**（新会话直入、或会话已轮替）→ 提示先运行 `/explore <Issue 编号>`（可带 change 名），停止——**不在本命令里访谈现想方案**（那是把探索欠的债转移到固化时还，方案质量失控的根源）
...
5. **输出**
   ...
   **artifact：** proposal.md / design.md / tasks.md（审查收敛，🟡 处置：修复 <X> / 豁免 <Y> 附理由；无则"无"）
   ...
   下一步：测试用例起稿 /test-draft → 对齐审查 /test-review → 运行 /apply <change-name> 开始实现。
   ```
...
- **只固化，不探索**——方案必须产自同会话的 /explore；无探索结论不开工（这是硬前置，不是建议）
