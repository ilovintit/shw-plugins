---
name: shw-do
description: "Codex prompt workflow for shw-do、/shw-do. 微活直通执行——直接执行 Issue 内容（单文件单点、不碰逻辑结构），不进 change 族。适用：直接执行、快速修复、一行修改、微活。主 agent 直写代码 + 单轮自审 + 提示 /shw-wrap 收尾 Use only when the user explicitly asks for this named workflow."
---

# shw-do

这是共享源码中同名 slash command 的 Codex skill-backed prompt，不是 Codex 原生 commands。用户明确点名 `shw-do`、`/shw-do` 或要求执行该工作流时按下方原文执行。
...
- 建分支 `feat/N-slug`（bug 用 `bug/N-slug`；线上紧急修复用 `hotfix/N-slug`——从 main 切，其余一律从**最新 dev** 切）
- **worktree 强制（无判定、无例外，见 `shw-worktree`）**：动手前一律先建/进 issue worktree。宿主有原生 Worktree 优先用原生入口，并把起点固化为 `feat/N-slug`；否则 fallback 为 `git worktree add ../<repo>-issue-N -b feat/N-slug dev`，hotfix 基点为 main。主目录只做 dev 基座，"孤立串行"不构成跳过理由，agent 不做例外自判（豁免仅用户明示，汇报注明）；发现已在主目录动了手 → 停手，把改动迁移进 worktree（stash / patch 搬运）再继续
