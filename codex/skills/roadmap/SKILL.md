---
name: roadmap
description: "Codex prompt workflow for roadmap、/roadmap. Milestone 执行驱动——读取既有 Milestone（/milestone 产出的计划），逐 Issue 驱动 change 族（explore → propose → apply → archive）直到全部 Issue 关闭、PR 全绿。不自建任何 Issue/Milestone Use only when the user explicitly asks for this named workflow."
---
...
**本命令不自建任何 Issue/Milestone**——Milestone 是 `/milestone` 的产物，本命令只消费它。计划终于 milestone，执行始于 roadmap。
...
- 测试用例前置检查：`e2e/`、`tests/vrt/` 已有经 `/test-review` 放行的用例？（正常流程 milestone → test-draft → test-review → 本命令。）**无已审用例 → 提示先走 test 族**，停止——执行期的测试必须是审过的
...
- 进 worktree（或换下一个 Issue）后第一步：`git checkout -b feat/<N>-<slug> origin/dev`——常驻 worktree 的分支轮转 = 每次 Issue 接力都创建新分支（见 shw-worktree「进 worktree 后第一步 = 固化 issue 分支」）
...
1. **explore**（`/explore <N>`）：差距调查——Issue 需求 × docs 方案 × 代码现状 → 差距清单 + 执行路径
2. **propose**（同会话紧跟）：固化三件套到 `.changes/<N>-<slug>/`
3. **apply**（`/apply`）：测试用例已在（test 族起稿、已审）——实现让用例转绿，并行子智能体执行，提 PR（`Closes #N`）
4. **等 CI 红绿循环**：编排模式由本命令盯 CI run——绿 → 合并**进 dev**（编排模式 agent 可自动合 dev；agent 永不合 main）；红 → 读日志修复再推，循环到绿
5. **archive**（`/archive`）：轨迹回写 Issue 评论、清理本地草稿
...
- 提示下一步：`/review`（双模型对抗终审，范围带上本 Milestone）→ 终审 🔴 清零后走 `/release` 放行（建 dev→main 放行 PR 列 Milestone Issue 清单，用户确认合并）→ main 打 tag 发布；**线上验证通过后才关 Milestone**（Sprint Review 核对时确认，口径见 shw-gitea-flow §5）
...
- **探索与固化同会话衔接**——explore 结论不落盘，propose 必须紧跟着跑（见 /explore、/propose）
