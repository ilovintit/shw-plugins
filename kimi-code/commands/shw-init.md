...
1. 创建 `.changes/`（工作区，不进 git）和 `specs/`（交付物，git 跟踪）目录，并把 `.changes/` 与 `.roadmaps/` 幂等写入 `.gitignore`（`.roadmaps/` 目录由 /shw-roadmap-explore 按需创建，init 不创建该目录）
...
2. **创建目录结构**
   ...
   守卫已确认 `.changes/` 不存在，直接创建：
     ...
     touch specs/.gitkeep
     ```
     并将 `.changes/` 与 `.roadmaps/` 一起幂等加入 `.gitignore`（工作区不进 git）：
     ```bash
     # 检查 .gitignore 是否已有对应条目，避免重复
     # .roadmaps/ 是 roadmap 本地探索工作区（由 /shw-roadmap-explore 使用），init 不创建该目录、仅前置写入
     grep -qxF '.changes/' .gitignore 2>/dev/null || echo '.changes/' >> .gitignore
     grep -qxF '.roadmaps/' .gitignore 2>/dev/null || echo '.roadmaps/' >> .gitignore
     ```
...
3. **处理 AGENTS.md**
   ...
   **情况 A：已有 AGENTS.md**
   ...
   - `/shw-archive` 归档 change（含 delta spec 同步到主 spec）
...
4. **显示总结**
   ...
   **AGENTS.md：** ✓ 已追加/创建（change 管理说明）
   ...
   3. 用 /shw-apply 实现
   4. 用 /shw-archive 归档 change（内含 spec 同步）
   ```
