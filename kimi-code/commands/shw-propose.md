...
6. **审查 change 与探索上下文的一致性**

   所有 artifact 创建完毕后，加载 `shw-review-change` skill 做逐字符审查。

   该 skill 会把 proposal/specs/design/tasks 与 explore 阶段讨论的上下文逐条对比，检查：
   - 需求覆盖完整性（explore 讨论的需求是否都在 artifact 中体现）
   - 方向一致性（技术选型/范围/优先级是否与讨论一致）
   - 内部自洽性（artifact 之间的交叉引用是否完整）
   - 表述精确性（是否有模糊/歧义/自相矛盾）

   若有 🔴 项（必须修复），修改 artifact 后重新审查，直到通过。

**输出**

完成所有 artifact 并审查通过后，总结：
- change 名和位置
- 创建的 artifact 列表及简短描述
- 审查结果（通过 / 已修复问题）
- 准备就绪内容："所有 artifact 已创建并审查通过！可以开始实现。"
- 提示："运行 `/shw-apply` 开始实现。"
