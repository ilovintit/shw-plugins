...
6. **循环：审查 -> 修复 -> 复审，直到收敛**
   ...
   加载 `shw-review-change` skill，对本次 propose 的全部 artifact（proposal/specs/design/tasks）做逐字符审查。审查维度（由 skill 定义）：
   ...
   ```
   loop:
     1. 加载/复用 shw-review-change skill 审查全部 artifact
...
### 自我审查结果
<shw-review-change 审查摘要，含 🔴 项的修复记录>
