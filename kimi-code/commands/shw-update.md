...
2. **逐条应用迁移清单**
   ...
   **清单条目只作用于 AGENTS.md 的「## change 管理」段落内**（init 写入的模板范围）——用户在段落外手写的内容绝不动；段落内的用户自定义行不在检测模式内的也不动。
...
- **只动 init 写入的范围**：AGENTS.md 的修改严格限定在「## change 管理」段落内且仅命中检测模式的行；`.changes/` 下只处理清单明确列出的文件
