---
description: 插件升级后的存量项目迁移 - 按迁移清单幂等检测并修复项目 AGENTS.md 等 /shw-init 写入内容的过时条目（死链 / 旧口径 / 作废文件），并维护 .roadmaps/ 的 git 忽略口径。仅已初始化项目需要；新项目直接 /shw-init；本命令不创建 change（日常开发用 /shw-propose）
---
