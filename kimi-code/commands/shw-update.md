...
加载 `shw-gitea-ci/references/project-template.md` 做模式、目录和 Workflow 对照审计。先读取现有 `.shw/project.yaml`：已有模式默认复用，不自行切换；用户明确提出新模式或要求按新默认重新对齐时，才完成差异审计、目标模式迁移、标识覆盖和 Issue 回写。无模式时必须让用户选择后再接入 SHW。发现 `spec/` 或 `specs/` 时，完整盘点其内容，将有效事实迁移并回写当前 Issue/对应真相文档，完成引用检查后删除目录；不得只改名或原样保留。发现公共 Taro UI、shw-ui 或 Go 库以 submodule、源码副本、本地路径依赖或 `go.mod replace` 引用时，先核对目标端正式发布包与兼容性；已有版本可用时迁移到私有 npm/Go 源正式版本并同步 lockfile/CI，尚未发布的 shw-ui 端包不得虚构或提前替换可运行旧依赖。

更新还必须识别项目是否存在真实 test 环境，并把小程序上传器、配置与 workflow 一起迁移为唯一匹配的二分模式：无 test 时 `channels`、push branches 和 job if 全部是 dev/main，dev 同时承担本地开发者工具和体验版来源；有 test 时三处全部是 test/main，dev 仅供本地开发者工具、test 提供体验版。不得保留 dev/test/main 混合 channels 或 workflow 分支集。API/E2E/VRT 不进入开发、dev→main 或 tag 门禁；已有基线比较与最终验收后的维护按独立入口处理。
...
## v7.7.0（#317）：逐端原生前端与 shw-ui 支持边界

检测：项目 Agent 规范、PRD、原型、架构或 CI 仍把 Taro 一份代码编译 H5/小程序视为新端默认；旧 `@shwkj/taro-ui`、submodule/源码包被当作 shw-ui 已发布端包；H5/小程序共用产物、API/路由和验收；iOS/Android/鸿蒙被要求共享 Shw 组件、CSS token 或相同交互。逐个盘点实际端、已发布包与 lockfile、构建脚本、开发者工具根、上传配置、部署和回归入口。

修复：按真实端及用户确认的迁移范围分别规划 H5 和每类小程序原生工程，并为三种原生 App 保留独立交互/技术选择；用该端已验证的 shw-ui 包时同步源码、锁文件、CI、图标、构建产物、部署和人工验收。尚无正式包或目标端尚未实现时，保留可工作的 Taro 依赖与流水线、登记迁移缺口，不机械删除旧配置或声称新端已验收。Taro 项目迁移须有本项目独立 Issue/工作区/PR；新项目不再生成 Taro 默认工程。App 规范归属 shw-ui 或另建库待实际启动时裁决。

## v7.6.0（#297）：API/E2E/VRT 迁移为最终人工验收后的回归基线

检测：开发前要求三类用例、自动化映射、CI RED 或截图；功能 Issue 与基线同 PR 强绑；Issue→dev、dev→main、tag/release 等待三类测试；旧 release/full-test/聚合 job 混合工程检查与 API/E2E/VRT；自动 baseline-update、回归失败重录、普通 push 具基线写权限；删除 job 后分支保护仍保留旧 required context；现有测试或图片没有验收来源；轻量模式被强制套用业务生命周期。

修复：先保留全部现有测试、expected 与历史记录，解除开发/发布前置，再按真实目的拆成工程检查、dev 对已有基线的独立非阻断比较、最终人工验收后的显式 `/shw-baseline` 维护三条路径。工程 required checks 保留构建、lint/typecheck、适用单元/协议/安全检查；回归报告真实区分差异、无基线、partial、执行错误和目标漂移，但不汇入 required/release gate。基线写入必须绑定不可变 acceptedSourceSha、制品/部署 revision、最终验收引用与 accepted scope，生成候选经审查和同目标复跑后走独立 chore Issue/PR。

逐项检查并修复 workflow/job、聚合脚本、分支保护 required contexts、Agent 规范、PRD/架构、test-plan、release/version-close 与 init 模板。混合 job 先拆出工程检查，删除旧 context 前核对保护规则，避免永久 pending。来源不明的旧基线标 `legacy-unverified`，不删除、不覆盖、不声称人工确认；历史失败/成功保持原意。无既有测试目录约定时才新增 `tests/regression/baseline-manifest.json`。仓库设置只在目标项目获授权时修改，无权限则准确列出未完成项；不批量改其他仓库。

## 待发布（#282）：退役全局端口登记

检测：项目 Agent 规范、初始化说明、开发脚本或文档仍要求加载 `shw-port-manager`，或把跨项目全局端口登记表作为启动本地服务的强制前置。

修复：移除 `shw-port-manager` 引用和全局端口登记门禁；保留项目自身已有的端口配置，不为迁移改号，也不修改或清理项目外的历史登记表。偶发的单程序本地运行按项目现有入口启动，实际发生端口占用时再在当前项目内调整并验证。

## 待发布（#195）：统一图标源与公共 CI 工具

检测：项目直接依赖 `lucide-static`，复制 `tools/font-icons/`、`svgtofont` 或其他字体生成器；页面散落 `@font-face`、codepoint、Emoji、字符或 SVG 图标；移动端 TabBar PNG 手工维护、普通态/选中态图形不同，或没有与页面图标共享语义映射；项目 Agent 规范或流程把新增图标、Lucide 快照/tag 更新委托给业务项目或仅作存储的 `shw-forks/lucide`。

修复：加载 `shw-frontend-spec-common/references/icon-management.md`。盘点每个真实端的现有图标名称、字体资源、页面引用和 TabBar 配置，再建立 `deploy/icons/config.json`：业务语义名只映射到已由 `shw-plugins` 批准的 Lucide 名称，明确端内字体/组件输出以及 TabBar 的输出、颜色和图标清单。删除重复工具前先由 `shw-ci-icons` 在该项目 CI 中生成并核验 WOFF2、CSS、组件与 PNG；页面改为统一 `IconFont`/`ShwIcon`。把新增图标与快照/tag 更新责任改为提交 `shw-plugins` 维护 Issue，`shw-forks/lucide` 仅保留批准快照存储职责。项目不安装 `lucide-static`；公共镜像版本和内部 Lucide tag 只能由插件维护者升级。静态资源发布凭据保留 Gitea Secret，禁止写入图标配置或产物。
...
修复：仅对尚在 Taro 且未获逐端迁移授权的旧项目维持 outputRoot=dist/${process.env.TARO_ENV}，同步 dist/h5、dist/weapp 的全部消费路径和单平台清理范围；保留正常项目、自动化连接和 watch，断连先重连，卡死恢复沿用已确认权限。逐端迁移后改用该端实际构建路径，不把本历史条目用于新项目。
...
修复：同步 shw-release-flow 的实际 model/provider 证据规则，缺失或回退停止放行；MCP 生成目标必填且校验，非 Codex 全部文本产物检查专属运行指令。空配置按 shw-k8s-observer/config.md 记录未配置与待宿主实测边界，不改用户配置、不虚构动态启用机制。
...
检测：任一 dev/test/生产 Fleet 源或渲染结果包含承载运行配置的 Kubernetes Secret/SecretList、`secretKeyRef`、`secretRef`、Secret volume、`secretName`，或文档仍要求通过 Kubernetes Secret 注入运行配置；dev 自建中间件参数依赖 Fleet 外人工配置，未随项目声明完整维护。按名称引用基础设施预置拉取凭据的 `imagePullSecrets` 合规，不计入本检测；仅当声明自行创建该 Secret 或写入凭据内容时判为违规。
...
检测：项目 Agent 规范缺少项目外只读约束，或指引 Agent 自动修改用户级配置、shell profile、插件缓存或外部共享库；检查脚本、符号链接和委派是否被当作绕过方式。
...
修复：按当前 flow/worktree/TDD/release-flow 全量重写对应工作流段；元数据操作按已有载体处理；标注产品部署适用性；前端选型按端规范、后端按锁定 lib 文档。不要把只有说明变更当作已完成宿主原生切换或真实安装验证；退役适配器不再迁移或提供本地分发路径。
...
- 移除 `.changes/`、`spec/`、`specs/` 等已退役流程说明，先完整盘点、迁移有效知识并回写 Issue/真相文档，再按本 Issue 规则删除；不因“历史资料”名义保留 `spec(s)` 目录。
