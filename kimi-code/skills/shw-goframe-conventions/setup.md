...
公司特定约定（DDD 四层、手工 DI、gerror+gcode 错误处理、decimal string）→ 指向 shw-goframe-conventions
...
- 问 PHP 到 Go 的迁移坑 → 见本 skill 的 migration.md（shw-goframe-conventions 附属文件）
...
| 公司 DDD 四层架构、手工 DI、gerror+gcode 错误处理（官方机制，不自建异常体系）、decimal string、命名约定 | **shw-goframe-conventions** | 公司特定规则 |
| PHP→Go 迁移陷阱（类型、空值、数组/切片、协程 vs Hyperfiber 等） | **migration.md**（shw-goframe-conventions 的附属文件） | 跨语言迁移专属 |

**判定原则**：

- 换一家公司这套东西还成立吗？成立 → 官方 skill；不成立 → 公司 skill
- 涉及"公司怎么规定"→ `shw-goframe-conventions`
- 涉及"PHP 老代码怎么对应 Go"→ shw-goframe-conventions 的 migration.md
