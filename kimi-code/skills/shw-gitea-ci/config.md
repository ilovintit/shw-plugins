...
- **pin 版本（当前 v1.7.0）随插件发版升级，升级 = 修改 backend-version.json + 重跑 capture + 发版，shim 自动对齐**；工具清单内嵌于 shim，注入与后端是否就绪无关（配置错误不影响工具列表，只影响调用）
...
Kimi Code 的插件清单没有配置面板，也没有 `${VAR}` env 插值机制，因此**必须**使用上方「全工具通用：env 文件」一节的配置方式。清单条目不携带 `env` 块——Kimi Code 的清单 `env` 会直接覆盖父进程环境继承，写占位符只会让它变成字面量并覆盖用户的真实环境变量。

清单条目形如（后端 pin 版本不在清单条目里，随插件构建配置固定；文中出现的 `vX.Y.Z` 均为占位示例，**实际固定版本见 src/plugin/mcp-shim/backend-version.json**——升级 = 修改 backend-version.json + 重跑 capture + 发版，shim 启动时自动对齐新版本后端）：

```json
{
  "gitea": {
    "command": "sh",
    "args": ["./mcp-shim/launch.sh"],
    "cwd": "./"
  }
}
```

`cwd` 把 `args` 里的入口锚定到插件根。Kimi Code 只接受以 `./` 开头且位于插件根内的 `cwd`，不合规时**静默忽略**该条目且不报错——所以配置后必须实际确认 `gitea` 条目在线，不能以清单存在推定连接可用。

launcher 读取 env 文件寻址与认证，token 的创建方法见下文「创建 Gitea token」。在启动 Kimi Code 的环境里导出 `GITEA_HOST` / `GITEA_ACCESS_TOKEN` 也是兼容路径（launcher 的优先级为工作区文件 > 用户级文件 > 调用方已注入 env）。

### Codex：环境变量白名单承接

Codex 插件清单没有用户配置面板；优先使用上方通用 env 文件。`.mcp.json` 的 `env_vars` 白名单继续透传 `GITEA_HOST`、`GITEA_ACCESS_TOKEN` 和第 3 节的 shim 覆盖变量；在**启动 Codex 的环境**里导出仍是兼容路径：
...
> 本节是保底方案。优先使用 `.shw-plugins/gitea.env` 的工作区级覆盖；只有当工具的项目级 MCP 配置确实需要一条独立条目时才走「项目自建条目 + AGENTS.md 声明」路线。

### 第一步：项目自建 MCP 条目

在你所用工具的项目级 MCP 配置里自建一条条目，id 用区别于 `gitea` 的名字（如 `gitea-alt`）。自建条目与 shim 无关——`command` 由用户自选直连后端，下例 `go run` 形态要求机器有 Go（或改用自装二进制路径）：
...
- **`GITEA_MCP_CALL_TIMEOUT_SECS`（进阶覆盖）**：调整 `tools/call` park 超时（默认 25 秒，须低于宿主约 30 秒的工具调用预算，否则宿主先静默超时）
...
| 工具调用报后端不可得（code `-32002`） | 后端获取或握手失败（下载失败 / 网络不通 / 二进制不可执行），shim 正按退避重试 | 查网络 / 代理能否访问 gitea.com，稍后重试；仍不行设 `GITEA_MCP_BIN` 指向本地已有二进制（第 3 节） |
| 工具调用报 park 超时（code `-32001`） | 后端 25 秒内未就绪或未应答；已代理的请求会同时向后端发取消 | 写操作结果未知，先查询目标状态再决定是否重试；持续出现同 `-32002` 排查 |
| 工具调用报挂起过多（code `-32000`） | 挂起与在途请求达 256 上限 | 等在途调用返回后重试 |
| 工具调用报后端中断（code `-32003`） | 后端进程崩溃 | shim 会尝试重拉后端；写操作结果未知，**先查询 Issue/PR/评论等目标状态**，仅在确认未生效或调用具幂等保证后再重试。持续出现按第 3 节用 `GITEA_MCP_BIN` 换用自备二进制排除 |
