...
| 环境变量 | 来源（ZCode 插件配置） | 含义 |
|----------|----------------------|------|
| `GITEA_HOST` | `gitea_host` | Gitea 服务地址，**含协议头**（如 `https://gitea.songhuwan.com`） |
| `GITEA_ACCESS_TOKEN` | `gitea_token` | Gitea API token |

ZCode 清单条目形态（版本号 `vX.Y.Z` 为占位示例，**实际固定版本见插件构建配置**，升级随插件发版走）：
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `gitea_host` | `https://gitea.songhuawan.com` | 公司主 Gitea 地址，保持默认即可（必须含协议头） |
| `gitea_token` | 空 | Gitea API token，**首次使用必填**，创建方法见下 |

### Kimi Code 配置入口（mcp.json 覆盖）

Kimi Code 没有插件配置面板，上表的 userConfig 两项在 Kimi 环境不生效。gitea 的 host / token 改走 mcp.json：在**用户级** `~/.kimi-code/mcp.json` 写一条与插件清单**同名**的 `gitea` 条目，按 Kimi 的覆盖机制**整体覆盖**插件声明的条目，跨项目全局生效。

`~/.kimi-code/mcp.json`（不存在则新建）：
...
```json
{
  "mcpServers": {
    "gitea": {
      ...
      "env": {
        "GITEA_HOST": "https://gitea.songhuawan.com",
        "GITEA_ACCESS_TOKEN": "粘贴你创建的 Gitea token"
      }
    }
  }
}
...
- token 创建方法与 scope 勾选原则见下节（两种客户端通用）
...
```
1. 浏览器打开 Gitea Web（如 https://gitea.songhuawan.com）并登录
2. 右上角头像 → Settings → Applications
3. Generate New Token：填写备注名 → 勾选 scope → 生成
4. 立即复制生成的 token（只展示一次），填入插件配置的 gitea_token
```
...
当项目代码不在公司主 Gitea（自建 / 客户环境，host 或 token 不同）时，插件的默认 `gitea` 条目指向的是主 Gitea，**不能直接用**。

> 本节是保底方案：ZCode 项目级覆盖插件 userConfig 的能力尚未实测验证，故走「项目自建条目 + AGENTS.md 声明」路线。客户端能力验证可行后，本节可简化。

### 第一步：项目自建 MCP 条目

在项目的 agent 配置（ZCode 项目级 MCP 配置）里自建一条条目，id 用区别于 `gitea` 的名字（如 `gitea-alt`）：
...
```json
{
  "mcpServers": {
    "gitea-alt": {
      ...
      "args": ["run", "gitea.com/gitea/gitea-mcp@vX.Y.Z"],
      "env": {
        "GITEA_HOST": "https://gitea.example.internal",
...
- `GITEA_HOST` 填该项目实际的 Gitea 地址（含协议头）
...
本项目代码托管在独立 Gitea（https://gitea.example.internal），
gitea 相关操作一律用 MCP 条目 `gitea-alt`，
不要用插件默认的 `gitea` 条目（它指向公司主 Gitea）。
```
...
| 认证失败（401 / 403） | token 未配置、已失效或 scope 不足 | 检查插件配置 `gitea_token` 已填且未过期；核对 token scope 是否覆盖所需操作（第 1 节）；项目特殊 Gitea 检查条目指向的 token 是否属于该实例 |
| 连不上 host（超时 / DNS 失败） | 地址格式错误或网络不通 | 确认 `gitea_host` **含协议头**（`https://gitea.songhuawan.com`，裸域名会导致连接失败）；内网 / 自建 Gitea 检查 VPN 与内网连通性 |
| gitea server 启动失败 / 工具列表为空 | Go 未安装或版本过低 | 终端执行 `go version` 确认工具链可用；无 Go 环境走二进制路线（第 3 节） |
| 查到的是另一个 Gitea 的仓库 | 用错了条目（默认 `gitea` 指向主 Gitea） | 核对项目 AGENTS.md 的 gitea 条目声明，按第 2 节纪律切换到正确条目 |
