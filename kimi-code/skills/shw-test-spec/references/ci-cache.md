...
统一名称和迁移顺序见 [CI 配置契约](ci-config.md)。本规范覆盖快速门禁、合并后构建、发布、缓存维护及 Fleet 使用的依赖；应用制品存 Harbor 项目空间，基础/工具链/第三方镜像存 ci-cache。Fleet/Helm 不从公网运行时拉取 chart 或镜像；依赖先镜像并固定版本。
