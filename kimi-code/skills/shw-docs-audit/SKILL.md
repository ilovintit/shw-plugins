...
审计不是只出报告：`/shw-update` 应持续修复到机械差距清零；无法当场完成的真实缺口建立 Issue并明确延期。禁止根据文件存在推断内容合格，也禁止从实现猜测并伪造产品意图。

环境与 CI 审计必须覆盖 shw-release-flow/references/environments.md 和 shw-test-spec/references/ci-config.md：退役 person/rdev、dev 合并触发、端与服务影响清单、三环境中间件隔离、HTTP Ingress、内部依赖传递链、统一配置名及管理员迁移前置。
