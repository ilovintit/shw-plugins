...
LogProvider.Send(Message) → Result:
    // 验证码不明文入日志：日志会被收集、归档、扩散，明文验证码 = 登录凭证泄漏面扩大
    // 打码形态：只记长度，如 code=****(4)
    codeLen = length(params["code"] ?? "")
    log.info("[SMS LOG] phone=%s scene=%s code=****(%d)", phone, scene, codeLen)
...
**fail-fast 纪律：生产环境 `sms.default` 为 log 时拒绝启动（或启动即高优先级告警）。** `?? "log"` 兜底是给开发环境的便利，但生产漏配 `SMS_DEFAULT_PROVIDER` 时，它会静默把所有短信送进日志：验证码用户收不到、告警短信没人收，而系统"看起来一切正常"（不报错、不抛异常、`SmsSentEvent` 照常分发）——短信能力无声失效。启动时必须显式校验：`env == production && sms.default == "log"` → 拒绝启动（推荐，逼人把配置补对）或至少触发高优先级告警。
...
- **记录关键信息**：phone、scene、验证码长度（验证码场景，打码形态如 `code=****(4)`）。验证码**不明文入日志**——日志会收集、归档、扩散，明文验证码等于把登录凭证撒出去；开发调试确需明文时走临时开关且仅限本地环境，生产一律打码。
