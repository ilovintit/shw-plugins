...
把所需 verified 引用写入项目 dev 部署声明；Namespace、存储、资源和 ConfigMap 连接参数仍由项目代码维护。Fleet 声明禁止 Kubernetes Secret 与 `imagePullSecrets`；内部镜像拉取能力由集群节点或基础设施侧预先提供，不把仓库认证写入 Fleet 生成资源。项目 Agent 不修改 K8S；Fleet 完成部署。CI临时测试也可复用这些镜像，但实例和数据必须与 dev 隔离。
