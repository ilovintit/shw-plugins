...
| 分布式锁 | shw-redis-lock | ✅ 可用（Worker 内置互斥已用） | SET NX EX + Lua owner 释放 |
| 权限系统 | shw-rbac | ❌ 待迁移 | 8 张表 + 权限点代码化 + 5 档数据范围 |
| 编码生成 | shw-seqnum | ❌ 待迁移 | Redis INCR + DB 恢复（带锁）+ N 进制 |
| 验证码 | shw-otp | ❌ 待迁移 | Redis 四键协同 + 频率限制 + 防爆破 |
...
> **占位路径说明**：gf-lib 是公司内部私有仓库组件，包路径以公司内部私有仓库实际地址为准，此处示例用占位路径；使用前先向维护者确认真实 go get 地址。下文所有 `import` 中的包路径同理，均为占位示例，以实际地址为准。
...
```go
import "gf-lib/session"  // 占位路径，以公司内部私有仓库实际地址为准
...
```go
import "gf-lib/middleware"  // 占位路径，以公司内部私有仓库实际地址为准
...
| 验证码 | shw-otp | Redis 四键协同（码 + 频率 + 失败计数 + 锁定）+ 验证消费分离 |
