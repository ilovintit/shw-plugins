...
遵守 `shw-frontend-spec-common` 的图标铁律：禁止 Emoji，只允许 font-icon。统一 `<ShwIcon :name="<lucide语义名>" />`（classPrefix 默认 `shwkj`）。图标字体、`@font-face`、语义名称映射和组件入口由 `@shwkj/taro-ui` 集中维护并随包自包含，不依赖 CDN；业务页面不得散落 codepoint、字体地址或另建 `@font-face`。缺的图标用 lucide 语义名占位（不渲染但不报错），记入组件库 PROGRESS 待补，不在业务项目自造图标字体或用 Emoji、字符、图片、SVG 顶替。
