...
dev→main PR 对当前候选运行适用工程、来源与发布检查；API/E2E/VRT 不在其中，也不因失败、未完成或测试维护滞后进入“例外合并”债务。此前 Issue 或其他 PR 的结果不能替代当前候选检查；同一发布 PR 的纯历史同步只能按 `shw-test-spec/references/history-sync.md` 复用工程证据。Agent 不得代为合并、跳过、取消或伪造检查。人工测试决定是否提交 main，发布后的最终人工验收决定哪些行为可由 `/shw-baseline` 固化。
...
**shw-plugins 仓库自身的发布例外**：以仓库 `AGENTS.md` 为准，仅核验源码 main/tag、发布 CI 与 GitHub 公开分发（main/tag、三宿主市场清单和插件内容、GitHub Release assets）。Gitea Release 及其 `.tar.gz` 即使由现有 CI 生成，也不属于本插件 Agent 的独立核验或 Milestone 关闭门槛；不查询、下载或要求用户在浏览器确认。此例外不推广到业务项目、其他插件/库/CLI，也不把未检查的附件表述为已验证。
