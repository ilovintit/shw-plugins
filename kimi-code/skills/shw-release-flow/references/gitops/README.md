...
生产从人工合并 main 且通过既有 release 门禁的稳定 vX.Y.Z tag 发布；程序验证 tag 与 S 相等、S 属于 main。人工放行与发布验证由受保护 workflow 保证；程序不把“存在 tag”当成已完成全量测试或用户验收。
...
- 输出验证 YAML、资源身份、重复资源、镜像 digest；拒绝 Secret/SecretList 资源、明显私钥和遗留镜像占位符。凭据仍须通过 Secret 引用注入。该验证不是完整 Kubernetes API schema/业务兼容性检查，也不能证明任意 ConfigMap 文本无敏感信息；项目仍需审查与相应验证。
