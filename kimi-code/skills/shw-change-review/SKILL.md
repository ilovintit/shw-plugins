---
name: shw-change-review
description: 在 propose 完成后调用。逐字符审查 change 的 artifact（proposal/specs/design/tasks）是否与 explore 阶段讨论的上下文一致，确保没有遗漏需求、偏离方向或自相矛盾。
---
...
- **独立使用**（非 propose 流程内）：用户对 change 内容有疑虑时手动调用
...
- **有 🔴 项**：必须修复后才能进入 apply。propose 流程内由其审查循环直接修复 artifact 并复审（连续 5 轮不收敛才停下交用户决策）；独立使用（非 propose 流程内）时列出修复建议交用户决定。
...
- **独立使用时（非 propose 流程内）不修改 artifact**——只报告问题，修复交用户决定；propose 流程内由其审查循环直接修复 artifact 并复审。
