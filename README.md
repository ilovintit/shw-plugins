...
> 插件定位：**主要面向 ZCode 优化和适配**——ZCode 产物是完整能力（skills + commands + 2 MCP（shw-spec + gitea 官方）+ Stop 验证闸门 hooks）。Kimi Code 产物是通用适配（skills + commands + 2 MCP），不包含 ZCode 专属能力。
...
| `shw-verify-code` | 验证码（短信码 / 邮箱码）生成、校验、防爆破、频率限制 |
...
ZCode 产物自带 hooks（4.3.0 起）：agent 停止时自动对照活跃 change 的验证契约判定「完成」——串行执行全部验收命令 + 检查全部期望产物，未满足则在拦截预算内打回并注入打回话术。Kimi Code 产物不含 hooks（该工具不识别插件 hooks）。
...
| `gitea` | 官方 gitea-mcp（`go run` 外部拉起，插件不打包），Gitea 全能力（CI / PR / issue…） | 需配置：ZCode 在插件设置填 Gitea 地址与 token；Kimi Code 设 `GITEA_HOST` / `GITEA_ACCESS_TOKEN` 环境变量。token 在 Gitea Web → Settings → Applications 创建，机器需 Go（或装二进制，进阶路径见 skill 内 config.md） |
