...
本仓库是 **shw-plugins 的发布仓库**，存放 Kimi Code、Codex 与 Claude Code 的构建产物。源码在私有 Gitea，CI 自动构建并推送到本仓库。

> 插件定位：三个产物分发同一套 skills/commands，差别只在清单形态与凭据传递方式。Kimi Code 产物是 skills + commands + gitea 官方 MCP + Worktree MCP + Kubernetes stdio 薄壳，**清单无配置面板也无 `${VAR}` 插值**，凭据走 env 文件与父进程环境继承，配置指引随产物 README 分发。Codex 产物是 skills + gitea 官方 MCP + Worktree MCP + Kubernetes stdio 薄壳，并把共享 commands 映射为显式 skill prompts，不伪造原生 commands。Claude Code 产物是 skills + commands + gitea 官方 MCP + Worktree MCP + Kubernetes HTTP 直连，凭据走原生 userConfig 插值，是三者中唯一有插值机制的宿主。
...
### Kimi Code 客户端（repo 直装）

Kimi Code 的 repo 安装只认插件根清单，本仓库根目录的 `kimi.plugin.json` 即为该清单：

```
/plugins install https://github.com/ilovintit/shw-plugins
```

也支持固定分支、tag 或 commit（`/tree/<ref>`、`/releases/tag/<tag>`、`/commit/<sha>`）。

另有 `.kimi-plugin/marketplace.json`（version 2 catalog）作为补充入口，需要显式指定：

```
/plugins marketplace https://raw.githubusercontent.com/ilovintit/shw-plugins/main/.kimi-plugin/marketplace.json
```

catalog 不会被自动发现，也可用 `KIMI_CODE_PLUGIN_MARKETPLACE_URL` 覆盖默认目录。

> **安装后必须逐条确认 `gitea`、`worktree`、`kubernetes` 三个 MCP 在线**。Kimi Code 对不合法的 MCP 条目是静默忽略的，不会报错，清单里有不等于连上了。连接配置见下方「Kimi Code 配置」一节。
...
Claude Code 读取仓库根的 `.claude-plugin/marketplace.json` 发现发布仓库内的 `claude-code/` 插件（与其他宿主的清单文件互不干扰）：
...
**Kimi Code**：直接按 ref 安装该稳定 tag，无需先克隆：

```
/plugins install https://github.com/ilovintit/shw-plugins/releases/tag/v8.0.0
```

也可克隆到独立持久目录后 `/plugins install /absolute/path/shw-v8.0.0`。先卸载现有 shw-plugins 再安装目标版本，随后 `/plugins reload` 或重启会话确认版本，并逐条确认三个 MCP 在线。回退时以另一个已成功发布的 tag 重复这一步；不要改写发布仓库 main/tag 来回退单台客户端。本仓库未声称该宿主的安装/回退 UI 流程已经实测。
...
各宿主都要确认安装版本与目标快照清单一致、Gitea initialize/tools/list 正常、Worktree 七工具可见；只读检查既有登记不迁移/删除工作区。出现状态格式或宿主兼容性问题时保留配置和数据，恢复原已知可用版本并记录失败证据，不能通过删除工作区状态来完成回退。
...
├── kimi.plugin.json                 ← Kimi 根清单（./ 路径指向 kimi-code/）
...
│   ├── kimi.plugin.json
│   ├── worktree-mcp/（工作区管理器 launcher；平台二进制从 Release assets 校验下载）
│   ├── mcp-shim/（gitea shim launcher；平台二进制从 Release assets 校验下载）
│   ├── skills/
│   └── commands/
├── codex/                           ← Codex 插件产物
...
│   ├── worktree-mcp/（工作区管理器 launcher；平台二进制从 Release assets 校验下载）
│   ├── mcp-shim/（gitea shim launcher；平台二进制从 Release assets 校验下载）
│   ├── kubernetes-mcp/（Codex 传输薄壳 launcher；平台二进制从 Release assets 校验下载）
│   └── skills/（32 个共享 skills + 17 个 command prompt wrappers）
...
Kimi Code 与 Claude Code 直接安装这 17 个 commands（Claude Code 以插件命名空间调用，如 `/shw-plugins:shw-work`）。Codex 插件清单没有原生 commands 字段，构建器把同名 command 映射为显式触发的 skill-backed prompt。
...
| `/shw-test-plan` | 建立人工验收计划并记录未来回归候选 |
| `/shw-work` | 完成一个 Issue |
...
| `/shw-baseline` | 最终人工验收后创建或同步不可变版本的回归基线 |
...
默认 root 为 `${XDG_DATA_HOME:-$HOME/.local/share}/shw-plugins/worktrees`；可通过 `SHW_WORKTREE_ROOT` 或 `${XDG_CONFIG_HOME:-$HOME/.config}/shw-plugins/worktree.json` 配置。Kimi Code 无配置面板，`SHW_WORKTREE_ROOT` 只能从父进程环境继承。参数/恢复详见分发的 shw-worktree/config.md 与 operations.md。
...
| MCP | 作用 | 是否需要配置 |
|-----|------|------------|
| `worktree` | 本地 Go 工作区管理：七工具、统一目录、自动登记、安全清理 | 需要 Git，不需要 Go 或新凭据；根目录可选配置，详见 shw-worktree/config.md |
| `gitea` | 官方 gitea-mcp（经插件自带 shim 网关拉起：工具面毫秒注入，后端按 pin 版本自动下载/监督/对齐，放用户级缓存），Gitea 全能力（CI / PR / issue…） | 机器无需 Go；所有工具可使用用户级/工作区 env 文件；Claude Code 支持插件设置（userConfig，安装/启用时提示），Kimi Code/Codex 的 env 文件与 shell 导出兼容；首次调用需访问 gitea.com 下载后端（数秒）；Windows 暂不支持（可用覆盖配置为自装二进制兜底） |
| `kubernetes` | 组织部署的统一多集群只读 Streamable HTTP MCP（项目 Agent 只用于 context、工作负载、Events、rollout 和日志观测） | Kimi Code 与 Codex 随插件分发 stdio 薄壳，自动读取稳定用户级 kubernetes.env，无需额外注册；不运行 Kubernetes 服务。Claude Code 使用插件设置（userConfig 插值）；完整示例在随插件分发的 `shw-k8s-observer/config.md`。URL 与 Agent 专属 Bearer 不得写入仓库或版本化插件路径。 |
...
API/E2E/VRT 只保护最终人工确认的回归基线：开发和发布仅等待适用工程检查；dev 对已有基线运行独立非阻断比较并真实保留差异、无基线、partial、环境错误与目标漂移；最终验收后由 `/shw-baseline` 绑定不可变 source、制品/部署 revision 与验收范围，通过独立 chore PR 维护。普通回归、push、release 都没有重录权限。
