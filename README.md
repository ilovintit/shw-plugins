...
│   └── skills/（31 个共享 skills + 16 个 command prompt wrappers）
...
ZCode 与 Claude Code 直接安装这 16 个 commands（Claude Code 以插件命名空间调用，如 `/shw-plugins:shw-work`）。Codex 插件清单没有原生 commands 字段，构建器把同名 command 映射为显式触发的 skill-backed prompt。
