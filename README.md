...
```bash
codex plugin add shw-plugins@shw-plugins-market
```

Desktop 用户也可重启 ChatGPT desktop 后，在 Plugins Directory 中选择该 marketplace，安装 `shw-plugins`。
...
└── codex/                           ← Codex 插件产物
    ...
    ├── README.md
    ├── mcp-shim/（gitea shim 网关：launch.sh + 三平台二进制）
    └── skills/（27 个共享 skills + 24 个 command prompt wrappers）
```

## 包含的 Commands

ZCode 与 Kimi Code 直接安装这 24 个 commands。Codex 插件清单没有原生 commands 字段，构建器把同名 command 映射为显式触发的 skill-backed prompt：用户点名 `/shw-*`、`shw-*` 或该工作流时执行，不新增私有 commands 机制。
...
| MCP | 作用 | 是否需要配置 |
|-----|------|------------|
| `gitea` | 官方 gitea-mcp（经插件自带 shim 网关拉起：工具面毫秒注入，后端按 pin 版本自动下载/监督/对齐，放用户级缓存），Gitea 全能力（CI / PR / issue…） | 机器无需 Go；ZCode 在插件设置填地址与 token；Kimi Code 在 shell 导出 `GITEA_HOST` / `GITEA_ACCESS_TOKEN`（进阶用 mcp.json 覆盖，见上方章节）；Codex 用 `env_vars` 白名单承接启动环境变量（Desktop 不继承终端临时 export）；首次调用需访问 gitea.com 下载后端（数秒）；Windows 暂不支持（可用覆盖配置为自装二进制兜底） |
