...
| `/shw-roadmap-explore` | 大任务深度探索与 roadmap 生产（北极星/方向解法/终态描述） |
| `/shw-roadmap-verify` | 把终态描述原料转写为 verify.md 终态描述句集，定稿后产出单判据 goal 文本 |
| `/shw-roadmap-archive` | 归档已完结的 roadmap（完成核验收尾 / 放弃记理由） |
...
| `shw-change-sync-spec` | 把 change 的 delta spec 全量一致性合并到主 spec（全量读取 + 交叉影响 + 一致性核验） | 归档时用户确认同步后 |
| `shw-goal-drive` | 差距驱动循环执行 roadmap 的编排打法（判差距→拆 change→小循环→记轨迹） | 用 goal 驱动跑 roadmap 时 |
