...
本仓库是 **shw-plugins 的发布仓库**，存放 ZCode 和 Kimi Code 两种工具的构建产物。源码在私有 Gitea，CI 自动构建并推送到本仓库。

## 安装

### ZCode 客户端（通过 marketplace）

只有 ZCode（兼容 Claude 插件体系）使用 marketplace 概念：
...
Kimi Code 不使用 marketplace，直接用 URL 指向 `kimi-code/` 子目录安装：

```
/plugins install https://github.com/ilovintit/shw-plugins/kimi-code
```
...
├── marketplace.json                 ← ZCode 市场清单（只列 shw-plugins，source 指向 zcode/）
├── README.md                        ← 本文件
├── zcode/                           ← ZCode 插件产物
...
│   ├── commands/  (6 个)
│   └── mcp-spec/  (index.js)
└── kimi-code/                       ← Kimi Code 插件产物（不进市场，URL 直接装）
    ├── kimi.plugin.json
    ├── skills/
    ├── commands/
    └── mcp-spec/
